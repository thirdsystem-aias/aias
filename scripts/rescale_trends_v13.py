"""v0.13 Trends rescaling and per-wave aggregation (fresh-acquisition arm).

Reads bundle raw responses from v0.13's locked acquisition session
(/v13/data/trends_raw/{region}/), applies pivot rescaling per pre-reg sec.5.1,
slices into t1 and t2 wave windows, applies E1b eligibility and E5 sparsity
flags. Mirrors v0.12's rescale logic verbatim — only the input data and
bundle composition change.

Scope: v0.13 fresh-acquisition arm only (skincare + finance). The reuse arm
(PM, running, olive oil) is NOT re-rescaled here; its v0.12 processed CSVs
are loaded directly at the scoring step per pre-reg §5.3.

Multi-category. Bundle composition matches acquire_trends_v13.py exactly.
Each category has its own pivot:
    skincare: CeraVe (8 bundles)
    finance:  YNAB   (4 bundles)
...
Padding entries (per the v0.13 §5.2 padding rule — incumbent-tier brand
canonicals used as scale anchors) are flagged is_padding=True and SKIPPED
in per-brand aggregation. Their bundle queries are real brand queries
(e.g., NerdWallet's query in B2), but they contribute zero rows to that
brand's per-brand within-window aggregation — only the brand's canonical
bundle entry contributes.

Outputs:
    /v13/data/trends_processed/per_brand_per_day.csv
    /v13/data/trends_processed/per_brand_within_window.csv
    /v13/data/trends_processed/summary_<region>_<category>.txt

No SerpAPI calls. Pure post-processing on locked acquisition data.

Run:
    python3 ~/aias/scripts/rescale_trends_v13.py
"""
import csv
import json
import statistics
...
# ============================================================================
# Bundle composition (must match acquire_trends_v13.py exactly)
# ============================================================================

# Each member tuple: (canonical_name, is_padding). Position 0 is the pivot
# for that category and bundle. Padding entries flagged True are skipped
# in per-brand aggregation; their queries are still acquired but their
# rescaled values don't contribute to the padding-brand's measurement
# (only that brand's canonical-position entry in another bundle does).
CATEGORIES = {
    "skincare": {
        "pivot": "CeraVe",
        "bundles": [
            {"id": 1, "members": [
                ("CeraVe", False), ("Cetaphil", False), ("Neutrogena", False),
                ("La Roche-Posay", False), ("Vanicream", False)]},
            {"id": 2, "members": [
                ("CeraVe", False), ("Skinceuticals", False), ("Eucerin", False),
                ("Paula's Choice", False), ("Drunk Elephant", False)]},
            {"id": 3, "members": [
                ("CeraVe", False), ("Tatcha", False), ("The Ordinary", False),
                ("EltaMD", False), ("Bioderma", False)]},
            {"id": 4, "members": [
                ("CeraVe", False), ("Avène", False), ("Aveeno", False),
                ("First Aid Beauty", False), ("Augustinus Bader", False)]},
            {"id": 5, "members": [
                ("CeraVe", False), ("Youth to the People", False),
                ("La Mer", False), ("Estée Lauder", False), ("Lancôme", False)]},
            {"id": 6, "members": [
                ("CeraVe", False), ("Clinique", False), ("Kiehl's", False),
                ("Sunday Riley", False), ("SK-II", False)]},
            {"id": 7, "members": [
                ("CeraVe", False), ("Olay", False), ("Origins", False),
                ("Dermalogica", False), ("Murad", False)]},
            {"id": 8, "members": [
                ("CeraVe", False), ("Glossier", False),
                ("Beauty of Joseon", False),
                ("__pad_Cetaphil", True), ("__pad_Neutrogena", True)]},
        ],
    },
    "finance": {
        "pivot": "YNAB",
        "bundles": [
            {"id": 1, "members": [
                ("YNAB", False), ("Mint", False), ("Quicken", False),
                ("Empower", False), ("NerdWallet", False)]},
            # B2: Quicken Simplifi EXCLUDED_E1a per Phase B; replaced by
            # __pad_NerdWallet (DEVIATIONS Entry 2).
            {"id": 2, "members": [
                ("YNAB", False), ("Rocket Money", False),
                ("PocketGuard", False), ("Goodbudget", False),
                ("__pad_NerdWallet", True)]},
            {"id": 3, "members": [
                ("YNAB", False), ("EveryDollar", False),
                ("Monarch Money", False), ("Copilot", False),
                ("Lunch Money", False)]},
            {"id": 4, "members": [
                ("YNAB", False), ("Origin", False), ("Cleo", False),
                ("Tiller", False), ("__pad_Empower", True)]},
        ],
    },
}
...
# Wave windows per pre-reg §6 (inclusive on both ends; v0.13 inherits
# v0.12's wave windows verbatim per §5.3 back-fill design).
T1_START = datetime(2026, 4, 27)
T1_END   = datetime(2026, 5,  3)
T2_START = datetime(2026, 5,  4)
T2_END   = datetime(2026, 5, 10)

# n-floor per pre-reg §3.6a
N_FLOOR_HARD  = 10
N_FLOOR_ALIGN = 12

V13_ROOT = Path.home() / "aias" / "osf" / "v13"
RAW_ROOT = V13_ROOT / "data" / "trends_raw"
OUT_DIR  = V13_ROOT / "data" / "trends_processed"
...
print("H5/H6/H7 cross-category, H8 Mint diagnostic, pooled sensitivity).")
print()
print("v0.13 analysis frame loads:")
print("  - v0.13 processed CSVs (skincare + finance) from this run")
print("  - v0.12 processed CSVs (PM software + running + olive oil)")
print("    from osf/v12/data/trends_processed/ per pre-reg §5.3 reuse-arm.")
