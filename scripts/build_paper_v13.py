#!/usr/bin/env python3
"""Build the v0.13 SSRN paper PDF via pandoc + xelatex.

Inputs:
    ~/aias/papers/v13_ssrn_paper_draft.md   — source markdown (Carlito YAML)
    ~/aias/osf/v13/figures/*.pdf             — embedded figures

Outputs:
    ~/aias/papers/v13/v13_ssrn_paper.pdf     — final PDF
    ~/aias/papers/v13/preamble_v13.tex       — generated LaTeX preamble
    ~/aias/papers/v13/v13_ssrn_paper_build.md — patched source (figure paths)

Visual identity matches v0.7-v0.12 SSRN papers:
...
relative paths, but the v0.13 source lives in ~/aias/papers/ alongside the
v0.12 source. Build rewrites to figures/... and adds ~/aias/osf/v13/ to
pandoc's --resource-path search list.

Run:
    python ~/aias/scripts/build_paper_v13.py

To force a rebuild of the figures themselves first:
    python ~/aias/scripts/build_charts_v13.py && python ~/aias/scripts/build_paper_v13.py
"""
...
V13_DIR = PAPERS_DIR / "v13"
V13_DIR.mkdir(parents=True, exist_ok=True)

SOURCE = PAPERS_DIR / "v13_ssrn_paper_draft.md"
RESOURCE_BASE = HOME / "aias" / "osf" / "v13"  # figures live at RESOURCE_BASE/figures/

OUT_PDF = V13_DIR / "v13_ssrn_paper.pdf"
PREAMBLE = V13_DIR / "preamble_v13.tex"
BUILD_MD = V13_DIR / "v13_ssrn_paper_build.md"  # patched source
...
preamble = r"""% v0.13 SSRN paper preamble — matches v0.7-v0.12 visual identity
\usepackage{float}
...
cmd = [
    ...
    f"--resource-path={V13_DIR}:{RESOURCE_BASE}",
    str(BUILD_MD),
    "-o", str(OUT_PDF),
]
