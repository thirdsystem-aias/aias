"""v0.13 Phase A — Pivot validation for the two NEW categories.

Per pre-reg §4.2:
- Out-of-sample window: 2026-04-13 to 2026-04-19 (7 days; disjoint from t1/t2).
- Region: Worldwide.
- Primary pivots: CeraVe (skincare), YNAB personal finance (finance).
- Fallback pivots: Cetaphil (skincare), Quicken (finance).
...
- Hard fail (both primary and fallback fail) routes the category to the
  literature-grounded pivot decision logged in DEVIATIONS.md per §4.2.

Outputs per pre-reg §15:
    ~/aias/osf/v13/data/phaseA_test/{cat}_{role}_{pivot}_{ts}.json
    ~/aias/osf/v13/data/phaseA_summary.csv

This is PRE-acquisition validation. No git-tag check, no acquisition-timestamp
lock; Phase A precedes both. Pre-reg lock at `git tag v0.13-prereg` occurs
after Phase A passes and the §4.1 table is updated with the outcomes.
...
Run:
    ...
    python ~/aias/scripts/phaseA_validate_v13.py
"""
import csv
import json
import os
...
# ============================================================================
# Configuration (locked at v0.13 pre-reg §4.2)
# ============================================================================
...
OUT_DIR     = V13_ROOT / "data" / "phaseA_test"
SUMMARY_CSV = V13_ROOT / "data" / "phaseA_summary.csv"
...
# PASS criteria per pre-reg §4.2
MEAN_FLOOR     = 25.0
CV_CEILING_PCT = 25.0

# Pivot test plan. Order matters: primary first, fallback second per category.
PIVOTS = [
    ("skincare", "primary",  "CeraVe",   "CeraVe"),
    ("skincare", "fallback", "Cetaphil", "Cetaphil"),
    ("finance",  "primary",  "YNAB",     "YNAB"),
    ("finance",  "fallback", "Quicken",  "Quicken"),
]
...
def write_pivot_json(result):
    ...
    pivot_safe = result["pivot_canonical"].replace(" ", "_")
...
print("=" * 72)
print("v0.13 Phase A pivot validation")
print(f"Pre-reg §4.2: out-of-sample window {WINDOW}, region=Worldwide")
...
for category in categories_in_order:
    ...
    if f_result["phase_a_status"] == "PASS":
        ...
        print(f"  -> Category HARD FAIL: both primary and fallback failed; "
              f"per §4.2 default to literature-grounded pivot in DEVIATIONS.md")
    print()
...
if n_hard_fail > 0:
    ...
    print("Document hard fails in DEVIATIONS.md before proceeding to acquisition lock.")
    sys.exit(1)
else:
    print("All categories have a validated pivot. Next steps:")
    print("  1. Update pre-reg §4.1 table with Phase A outcomes (Status at lock).")
    print("  2. Update pre-reg §3.4 / §3.5 if fallback was selected.")
    print("  3. Commit: git add osf/v13/data/phaseA_*")
    print("  4. Lock pre-reg: git tag v0.13-prereg")
    print("  5. Run fresh-acquisition session (24 bundles).")
