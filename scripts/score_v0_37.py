#!/usr/bin/env python3
"""
score_v0_37.py — AIAS(TM) v0.37 CPC scorer.

================================ SEEDING NOTICE ================================
SEEDED VERBATIM FROM score_v1_8.py (v1.8 redesigned CPC instrument scorer,
v1.8 branch @ 85d2202) per the v0.37 plan. The reusable substrate carried
forward: the phi/J instrument machinery, the certified surfacing-extraction
lineage, the reconciliation gate, the loud-failure input discipline, and the
one-way --run boundary.

REWORK REQUIRED before this scorer is valid for v0.37. Everything hypothesis-
specific below — the SPEC (forks A-D), the H_* verdict logic, the two-wave
(v0.34) and framing-membership (v0.35) stages, and the constants read from the
content module at module top — is v1.8's CONSISTENCY-INSTRUMENT analysis.
v0.37's substrate is the IDENTITY-LOAD MODERATOR (a moderation design), a
different hypothesis structure; none of the H_* logic transfers unchanged. It
must be rewritten against v0.37's hypotheses once
prereg/v0_37_cpc_identity_load_moderator_content.py is AUTHORED and locked at
tag v0.37-prereg-r1. Until then this file does not resolve.

ROOT FOOTGUN: line ~62 hardcodes ROOT = ~/aias (program convention). v0.37
currently lives in the worktree ~/aias-v0.37; score only after the v0.37 branch
is checked out in the main tree (or adjust ROOT), else it reads the v1.8 tree.
===============================================================================
...
roster / v0.36 low-recall stratum). Produces:
    osf/v37/v37_verdicts.json          (per-hypothesis verdicts)
    osf/v37/v37_instrument_table.csv   (supplementary per-brand)
...
ONE-WAY BOUNDARY (program discipline):
    ...
    prose finding anywhere before v37_verdicts.json exists.
...
sys.path.insert(0, str(ROOT / "prereg"))                       # locked v0.37 module
...
import v0_37_cpc_identity_load_moderator_content as PREREG     # locked record (v0.37-prereg-r1; TO BE AUTHORED)
...
def write_outputs(table, verds, recon, two_wave, secondary, frame_resolved):
    ...
    with open(outdir / "v37_instrument_table.csv", "w", newline="") as fh:
...
def main(do_run):
    ...
    print(f"\nWrote {outdir/'v37_verdicts.json'} and {outdir/'v37_instrument_table.csv'}")
    return 0


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="AIAS v0.37 CPC scorer (one-way boundary; --run required).")
