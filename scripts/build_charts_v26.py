#!/usr/bin/env python3
"""
v0.26 chart builder — 4 figures for the brand-format report
============================================================
...
Figure 3 — chart_26_convergent_vs_discriminant.pdf
    ...
    next to v0.26 pooled C_P × BSR (discriminant, ρ ≈ 0).
...
# -----------------------------------------------------------------------------
# Figure 3 — convergent (v0.25) vs discriminant (v0.26) two-panel comparison
# -----------------------------------------------------------------------------
...
def build_convergent_discriminant():
    ...
    # Right — discriminant (v0.26 pooled across 4 substrates)
    cps26 = np.array([d[1] for d in v26_pairs], dtype=float)
    bsrs26 = np.array([d[2] for d in v26_pairs], dtype=float)
    ...
    axes[1].set_xlabel(r"$C_P$ (v0.26)", fontsize=cs.FONT_SIZES["axis_label"])
    axes[1].set_ylabel("BSR percentile by substrate", fontsize=cs.FONT_SIZES["axis_label"])
    axes[1].set_title("Discriminant: v0.26 pooled cross-substrate",
    ...
    cs.add_footer(
        fig,
        verdict="Pattern 3: v0.25 + v0.26 jointly satisfy Campbell–Fiske construct validity.",
        phase=PHASE,
    )
