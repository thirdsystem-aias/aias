#!/usr/bin/env python3
"""
build_paper_v0_22.py — AIAS v0.22 SSRN paper PDF builder.

Compiles papers/v0_22/v0_22_ssrn_paper_draft.md → papers/v0_22/v0_22_ssrn_paper.pdf
via pandoc + xelatex with the Carlito-tuned preamble carried in the .md YAML.

Pipeline (canonical, matches v0.20 / v0.21 / v1.6):
  1. Read source .md
  ...
  5. Output PDF to papers/v0_22/v0_22_ssrn_paper.pdf
...
The source .md carries:
  ...
  - Inline figure references at section 3 Results:
    ![cap](../../reports/figs/v22/chart_NN_<topic>.pdf){#fig:label width=100%}
...
Usage:
  python build_paper_v0_22.py                                   # default paths
  python build_paper_v0_22.py --keep-temp                       # retain pre-processed .md
  python build_paper_v0_22.py --source <path> --output <path>   # override paths
"""
...
PAPER_DIR = AIAS_ROOT / "papers" / "v0_22"

DEFAULT_SOURCE = PAPER_DIR / "v0_22_ssrn_paper_draft.md"
DEFAULT_OUTPUT = PAPER_DIR / "v0_22_ssrn_paper.pdf"
DEFAULT_TEMP = PAPER_DIR / "_v0_22_ssrn_paper_pre.md"

# Pandoc resource path — figure refs in the .md use ../../reports/figs/v22/...
...
def run_pandoc(source: Path, output: Path, resource_path: str) -> int:
    ...
    print(f"[build_paper_v0_22] invoking pandoc:")
    print(f"  $ {' '.join(cmd)}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AIAS v0.22 SSRN paper PDF builder (pandoc + xelatex + Carlito)"
    )
    parser.add_argument("--source", default=str(DEFAULT_SOURCE),
                        help="Source .md path. Default: papers/v0_22/v0_22_ssrn_paper_draft.md")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT),
                        help="Output PDF path. Default: papers/v0_22/v0_22_ssrn_paper.pdf")
    ...
    print(f"[build_paper_v0_22] source: {source}")
    print(f"[build_paper_v0_22] output: {output}")

    # Stage 1 — Unicode subs
    text = source.read_text(encoding="utf-8")
    text, sub_report = apply_unicode_subs(text)
    if sub_report:
        print(f"[build_paper_v0_22] Unicode → LaTeX substitutions applied:")
        for ch, count in sub_report.items():
            print(f"  U+{ord(ch):04X} ({ch}) × {count}")
    else:
        print(f"[build_paper_v0_22] no Carlito glyph-gap chars detected")
    ...
    if rc != 0:
        print(f"[build_paper_v0_22] pandoc returned {rc}", file=sys.stderr)
        return rc
    ...
    try:
        ...
        pass
    print(f"[build_paper_v0_22] ✓ PDF written: {output}  ({final_pages} pages)")
