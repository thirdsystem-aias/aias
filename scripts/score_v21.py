#!/usr/bin/env python3
"""
score_v21.py — AIAS v0.21 scoring runner.

Consumes the raw acquisition outputs from run_acquisition_v21.py and produces:
  ...
  7. Verdict resolution for the four pre-registered hypotheses:
       ...
       - H_IdentityLoad_moderator (5-leg joint; v0.21 leg only computed here)

Output: v21_verdicts.json at the OSF v21 directory.

Required pip packages:
    scipy   (for Spearman ρ)

Usage:
    python score_v21.py                          # default paths
    python score_v21.py --registry <path>        # override registry
    python score_v21.py --phase-a <csv> --phase-b <csv>   # override input paths
"""
...
DEFAULT_DATA_DIR = Path.home() / "aias" / "osf" / "v21"
DEFAULT_OUTPUT = DEFAULT_DATA_DIR / "v21_verdicts.json"
...
def resolve_h_regime4(cp_data: dict, c2: dict, il_guard: dict,
    ...
    return {
        "primary": False,
        "first_prospective_v1_5_C2": False,
        "verdict": verdict,
        "reason": reason,
...
def resolve_h_identity_load_moderator(h_regime4_verdict: dict, cp_data: dict) -> dict:
    """v0.21 leg of the 5-leg joint. Joint matrix uses prior legs from v0.16/v0.17/v0.18/v0.20."""
    # v0.21 leg outcome shape
    v21_verdict = h_regime4_verdict["verdict"]
    ...
    if v21_verdict == "CONFIRMED" and monotonic:
        joint_verdict = "CONFIRMED-bounded"
        reason = "v0.21 CONFIRMED with monotonic IL signature C→A→B"
    elif v21_verdict == "PARTIAL":
        joint_verdict = "PARTIAL"
        reason = "v0.21 PARTIAL (joint with v0.16 PARTIAL × v0.17 FALSIFIED-on-panel-inadequacy × v0.18 PARTIAL × v0.20 FALSIFIED)"
    elif v21_verdict == "FALSIFIED" and "panel inadequacy" in h_regime4_verdict.get("reason", "").lower():
        joint_verdict = "INDETERMINATE"
        reason = "v0.21 FALSIFIED on panel inadequacy; rerun on adequate panel"
    elif v21_verdict == "FALSIFIED":
        joint_verdict = "NARROWED"
        reason = "v0.21 FALSIFIED (not on panel inadequacy)"
    else:
        joint_verdict = "UNRESOLVED"
        reason = f"v0.21 returned {v21_verdict}; joint not classifiable"

    return {
        "primary": False,
        "joint_legs": ["v0.16 PARTIAL", "v0.17 FALSIFIED-on-panel-inadequacy", "v0.18 PARTIAL", "v0.20 FALSIFIED", f"v0.21 {v21_verdict}"],
        "v0_19_excluded_rationale": "uniform-IL design; no contribution to moderator",
        "verdict": joint_verdict,
        "reason": reason,
        "v0_21_leg_verdict": v21_verdict,
        "il_signature": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="AIAS v0.21 scoring runner")
    ...
    print("AIAS v0.21 scoring runner")
