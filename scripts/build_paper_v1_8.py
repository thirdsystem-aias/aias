#!/usr/bin/env python3
"""
build_paper_v1_8.py — AIAS v1.8 methodology paper PDF builder.

Compiles papers/v1_8/v1_8_ssrn_paper_draft.md → papers/v1_8/v1_8_ssrn_paper.pdf
...
This builder is for the v1.8 methodology paper — no chart figures, text +
...
Usage:
  python build_paper_v1_8.py
  python build_paper_v1_8.py --keep-temp
  python build_paper_v1_8.py --source <path> --output <path>
"""
...
PAPER_DIR = AIAS_ROOT / "papers" / "v1_8"

DEFAULT_SOURCE = PAPER_DIR / "v1_8_ssrn_paper_draft.md"
DEFAULT_OUTPUT = PAPER_DIR / "v1_8_ssrn_paper.pdf"
DEFAULT_TEMP = PAPER_DIR / "_v1_8_ssrn_paper_pre.md"

RESOURCE_PATH = str(PAPER_DIR)


# --- Unicode → LaTeX substitutions (Carlito glyph-gap defense) --------------
# Canonical dict from build_paper_v21.py; v1.8 source uses heavier math notation
...
def run_pandoc(source: Path, output: Path, resource_path: str) -> int:
    ...
    print("[build_paper_v1_8] invoking pandoc:")
    print(f"  $ {' '.join(cmd)}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AIAS v1.8 methodology paper PDF builder (pandoc + xelatex + Carlito)"
    )
    parser.add_argument("--source", default=str(DEFAULT_SOURCE),
                        help="Source .md path. Default: papers/v1_8/v1_8_ssrn_paper_draft.md")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT),
                        help="Output PDF path. Default: papers/v1_8/v1_8_ssrn_paper.pdf")
    ...
    print(f"[build_paper_v1_8] source: {source}")
    print(f"[build_paper_v1_8] output: {output}")

    text = source.read_text(encoding="utf-8")
    text, sub_report = apply_unicode_subs(text)
    if sub_report:
        print("[build_paper_v1_8] Unicode → LaTeX substitutions applied:")
        for ch, count in sub_report.items():
            print(f"  U+{ord(ch):04X} ({ch}) × {count}")
    else:
        print("[build_paper_v1_8] no Carlito glyph-gap chars detected")
    ...
    if rc != 0:
        print(f"[build_paper_v1_8] pandoc returned {rc}", file=sys.stderr)
        return rc
    ...
    try:
        ...
        pass
    print(f"[build_paper_v1_8] ✓ PDF written: {output}  ({final_pages} pages)")
