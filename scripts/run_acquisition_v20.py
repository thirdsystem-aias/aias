#!/usr/bin/env python3
"""
run_acquisition_v20.py — AIAS v0.20 acquisition runner (v2 — fail-fast auth).

Executes Phase A (Recognition) and Phase B (six-frame Recall) against the
locked six-slot reference panel per pre-registration v0.20-prereg-r1.

Phase A: 144 probes (24 brands × 6 models) with the locked C_P probe template.
...
Usage:
    python run_acquisition_v20.py                          # both phases
    python run_acquisition_v20.py --phase a                # Phase A only
    python run_acquisition_v20.py --phase b                # Phase B only
    python run_acquisition_v20.py --dry-run                # no API calls
"""
...
DEFAULT_OUTPUT_DIR = Path.home() / "aias" / "osf" / "v20"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AIAS v0.20 acquisition runner (Phase A + Phase B)"
    )
    ...
    print("AIAS v0.20 acquisition runner (v2 — fail-fast auth)")
    ...
    if registry.get("phase") != "v0.20":
        print(f"WARNING: registry phase is '{registry.get('phase')}', expected 'v0.20'",
              file=sys.stderr)
    ...
    if not args.dry_run:
        ...
        print(f"\n  Next steps:")
        print(f"    cd ~/aias")
        print(f"    git add osf/v20/phase_a_results.csv osf/v20/phase_b_results.csv")
        print(f"    git commit -m 'acquisition: v0.20 — Phase A + Phase B raw results'")
        print(f"    git tag v0.20-acquisition-locked")
        print(f"    python ~/aias/scripts/score_v20.py")
