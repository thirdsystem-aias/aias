"""v0.16 Trends rescaling and per-wave aggregation (kitchen knives).

Reads bundle raw responses from v0.16's locked acquisition session
(osf/v16/data/trends_raw/{region}/), applies pivot rescaling per v0.13
pre-reg §5.1, slices into t1 and t2 wave windows, applies E1b eligibility
and E5 sparsity flags. Mirrors v0.13/v0.14/v0.15 rescale logic verbatim —
only the input data, bundle composition, pivot, and brand count change.

Scope: v0.16 fresh-acquisition. Kitchen knives is the single category.

Bundle composition MUST MIRROR acquire_trends_v16.py exactly.

Pivot (Victorinox) appears in every bundle. To avoid double-counting,
its per-day row is sourced from bundle 1 only.

Outputs:
    osf/v16/data/trends_processed/per_brand_per_day.csv
    osf/v16/data/trends_processed/per_brand_within_window.csv
    (summary tables printed to stdout)

No SerpAPI calls. Pure post-processing on locked acquisition data.

Run:
    python3 ~/aias/scripts/rescale_trends_v16.py
"""
import csv
import json
import statistics
...
# ============================================================================
# Bundle composition (MUST match acquire_trends_v16.py exactly)
# ============================================================================
...
OUT_DIR  = V16_ROOT / "data" / "trends_processed"
...
for region in REGIONS:
    for category in CATEGORIES.keys():
        ...
        print("=" * 110)
        print(f"{'Brand':<30} {'t1_mean':>10} {'t1_sd':>8} {'t2_mean':>10} {'t2_sd':>8} "
        ...
        for row in sorted(cat_rows, key=sort_key):
            ...
            print(f"{row['brand']:<30} {row['t1_mean']!s:>10} {row['t1_sd']!s:>8} "
...
print("=" * 110)
print("Rescaling complete. Next: canonical scoring.")
print("Next: python3 ~/aias/scripts/score_v16.py")
