#!/usr/bin/env python3
"""
build_charts_v28.py — AIAS v0.28 (CV.04) figure builder.

Builds the four CV.04 discriminant-validity figures from the LOCKED artifacts:
  - osf/v28/data/v0.28_presence.csv   (v0.28-acquisition-locked)
  - prereg/v0_28_brand_validator.csv  (v0.28-prereg-r1)
  - osf/v28/v28_verdicts.json         (v0.28-results-locked)
...
in v28_verdicts.json. If the standardization (ddof) differs from score_v28.py, the
...
================ RECONCILE BEFORE RUNNING (assumptions I can't verify from here) ===
 ...
 3. Z_DDOF. score_v28.py's within-panel z standardization: 0 = population, 1 = sample.
...
# Within-panel z standardization. RECONCILE with score_v28.py (see header item 3).
Z_DDOF = 0
...
def fig_dissociation(rows, verdicts, fig_dir):
    ...
    if mismatches:
        ...
        sys.exit(
            ...
            "Not drawing a figure inconsistent with v28_verdicts.json."
        )
...
def fig_composition(rows, have_components, fig_dir):
    if not have_components:
        print("[warn] fig 4 skipped: presence.csv lacks C_P / R_cat / R_cult columns. "
              "Point COMPONENTS_FROM at a file that has them, or regenerate via bridge_v28.")
        return
...
# ----------------------------------------------------------------------------- #
def main():
    ap = argparse.ArgumentParser(description="Build the v0.28 (CV.04) figures.")
    root = Path("~/aias").expanduser()
    ap.add_argument("--presence", default=root / "osf/v28/data/v0.28_presence.csv")
    ap.add_argument("--validator", default=root / "prereg/v0_28_brand_validator.csv")
    ap.add_argument("--verdicts", default=root / "osf/v28/v28_verdicts.json")
