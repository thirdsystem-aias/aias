#!/usr/bin/env python3
"""
v0.29 Automotive — typesetting pipeline.
...
Brand-format report companion to the v0.29 SSRN paper (SSRN TBD).

Forked from build_report_v21.py with surgical changes:
  - Imports v29_automotive_content instead of v21_cosmetics_content
  - HERO_FIGURE_CAPTIONS rewritten as v0.29 templates (4 entries; the
    fourth — f4_phantom_defunct — is net new for v0.29)
  - _slot_lookup table updated for the 4 v0.29 brand-format chart filenames
    (all produced by build_charts_v29.py at reports/figs/v29/):
  ...
  - Deposit root: osf/v29/
  - Page count target band: 8-14 pages (v0.29 has 4 findings vs v0.21's 3;
...
Inputs:
  ...
  - reports/figs/v29/chart_01_*.pdf      (4 brand-format charts; all from
                                          build_charts_v29.py)

Output:
  - osf/v29/reports/v29_cv_baseline_report.pdf

Run for v0.29:
    python3 ~/aias/scripts/build_charts_v29.py
    python3 ~/aias/reports/build_report_v29.py
...
# Make `import v29_cv_baseline_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v29_cv_baseline_content as content  # noqa: E402
import tsboilerplate as boilerplate  # noqa: E402

# ---------------------------------------------------------------------------
# Content adapter — reshape flat v0.29 content module into builder structures
# ---------------------------------------------------------------------------
...
if "tagline" not in content.COVER:
    content.COVER["tagline"] = "AIAS™ Measurement Program · Construct-Validity Baseline"
...
if isinstance(content.HYPOTHESIS_SCORING, list) and content.HYPOTHESIS_SCORING and isinstance(content.HYPOTHESIS_SCORING[0], dict):
    ...
    content.HYPOTHESIS_SCORING = {
        ...
        "intro": "Four pre-registered conditions on the inherited convergent and discriminant verdicts; the baseline is CONFIRMED only if all hold jointly.",
        "rows": _rows,
    }
...
_AKKURAT_SUBS = {
    "\u2227": "&amp;",   # logical-and -> ampersand entity
    # Akkurat lacks Greek + subscripts; substitute to ReportLab markup so
    # they render instead of tofu (CV.05 prose uses |rho| and C_3).
    "\u03c1": "<font name='Helvetica'>\u03c1</font>",  # rho: render in Helvetica (Akkurat lacks the glyph); keep it rho, not r (Spearman, not Pearson)
    "\u2083": "<sub>3</sub>",   # subscript-3 (C3)
    "\u2081": "<sub>1</sub>",   # subscript-1
    "\u2082": "<sub>2</sub>",   # subscript-2
...
CHART_FIGSIZE_IN = {
    ...
    # v0.29 B2B SaaS brand-format report keys — 4 charts.
    "6_col_v29_recall_by_brand":  (7.50, 9.00),
    "6_col_v29_mlt_frequency":    (7.50, 5.50),
    "6_col_v29_identity_load":    (7.50, 5.50),
    "6_col_v29_mlc_by_model":     (7.50, 5.50),
    "6_col_v29_phantom_gap":      (7.50, 5.00),
    "hero":                       (7.50, 5.00),
...
class ChartReservation(Flowable):
    def __init__(self, slot_key: str, chart_path: Path,
        ...
        # v0.29: auto-size reservation height to match the actual chart PDF's
        # aspect ratio. bbox_inches='tight' in build_charts_v29 produces
    ...
    def wrap(self, available_w, available_h):
        ...
        if self.caption_p is not None:
            ...
            total_h = self.chart_h_pt + ch + 2   # was + 4 (v0.29 tightening)
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename: str, *,
        ...
        # v0.29 lead-page layout — REVERTED to v0.21 / aias_1_0 working pattern:
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Construct-Validity Baseline \u00b7 v0.29 \u00b7 June 2026")
...
HERO_FIGURE_CAPTIONS = {
    "f1_mtmm_gap": (
        "Figure 1 \u00b7 The construct-validity gap. Convergent correlation "
        "magnitude (|\u03C1| = 0.74, significant) against discriminant magnitude "
        "(|\u03C1| = 0.0002, non-significant, inside the pre-registered 0.20 "
        "ceiling). The gap C\u2083 = 0.74 is the single quantity this synthesis "
        "computes; a positive gap with the significance asymmetry intact is the "
        "Campbell\u2013Fiske evidence that the Presence component is construct-valid."
    ),
    "f2_convergent": (
        "Figure 2 \u00b7 The convergent leg. AIAS Presence against Google Trends "
        "search interest across the 24-brand B2B SaaS registry (v0.25), Spearman "
        "\u03C1 = 0.74. The discriminant relationship (Presence vs. Amazon Best "
        "Sellers Rank, \u03C1 = \u22120.0002, n = 88) is the inherited v0.26 contrast, "
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.29 (CV.05) brand-format slot_key to (filename, figsize_key).
    Charts produced by build_charts_v29.py at reports/figs/v29/."""
    table = {
        "f1_mtmm_gap": (
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.29 protocol version — v1.6 (Phantom Brand Persistence increment Inc3)
...
def overlay_charts(base_pdf_path: Path, manifest: ChartManifest,
    ...
    for slot in manifest.slots:
        ...
        # v0.29: scale = min(sx, sy) — was * 0.92 (8% safety margin). The
        ...
        # matched to build_charts_v29.py figsize, the margin is unnecessary.
        scale = min(sx, sy)
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v29] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v29] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v29"
    # Charts live in reports/figs/v29/ (matches build_charts_v29.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v29")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v29_cv_baseline_report.pdf")
    base_pdf = output_path.parent / "_v29_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v29] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "f1_mtmm_gap",
        "f2_convergent",
        "f3_discriminant",
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v29] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc.build(story)
    print(f"[build_report_v29] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v29] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v29] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v29/reports/v29_cv_baseline_report.pdf")
    args = ap.parse_args()
    build(
