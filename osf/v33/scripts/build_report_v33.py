#!/usr/bin/env python3
"""
v0.33 Provider-Asymmetric CPC — brand-format report typesetting pipeline.

Third System managerial-register companion to the v0.33 SSRN paper. Renders the
P1-P5 propositions (P1/P2/P3 charted, P4/P5 text-only) from
v33_provider_asymmetry_content.py, with three report-register figures from
build_charts_v33.py (report mode):
...
Charts live in reports/figs/v33/report/; output to osf/v33/reports/.

Run:
    python3 ~/aias/scripts/build_charts_v33.py     # both registers
    python3 ~/aias/reports/build_report_v33.py
"""
...
# Make `import v33_provider_asymmetry_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v33_provider_asymmetry_content as content  # noqa: E402
...
# v0.33: the chart-bearing findings ARE the P-propositions (P1/P2/P3 charted,
...
# HYPOTHESIS_SCORING (v0.33): [{id, proposition, verdict, basis}] → {heading, intro, rows}
...
# HYPOTHESIS_DETAILS (v0.33 dict P1-P5) is consumed into the P-findings above;
...
if isinstance(content.CLOSING, str):
    content.CLOSING = {
        ...
        "datasets": [
            "Per-brand CPC: osf.io/ec6wh/v33/data/v33_cpc.csv",
            "Verdicts: osf.io/ec6wh/v33/v33_cpc_verdicts.json",
            "Scoring code: osf.io/ec6wh/v33/scripts/score_v0_33.py",
        ],
        "methodology_log": "v0.33 (SSRN pending)",
...
if isinstance(content.CLOSING, dict):
    ...
    content.CLOSING.setdefault("methodology_log", "v0.33 (SSRN pending)")
...
CHART_FIGSIZE_IN = {
    ...
    # v0.33 provider-asymmetry report figures — native dims from report_fig_0*.pdf mediaboxes
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "AIAS\u2122 Consistency (CPC) \u00b7 v0.33 \u00b7 June 2026")
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    citation_text = (
        ...
        "AIAS Consistency Component (CPC), v0.33</i>. "
        "Third System (SSRN pending). thirdsystem.ai/v33"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v33] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v33] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v33"
    # v0.33 figures live in reports/figs/v33/ (build_charts_v33.py default
    # fig-dir), the same chart_0*.pdf the SSRN paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v33" / "report")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v33_provider_asymmetry_report.pdf")
    base_pdf = output_path.parent / "_v33_base.pdf"
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v33] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="Consistency, Category by Category \u2014 AIAS v0.33",
    ...
    doc.build(story)
    print(f"[build_report_v33] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v33] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v33] page count: {final_pages}")
