#!/usr/bin/env python3
"""
v0.17 Premium Kitchenware — typesetting pipeline.

Recognition x Recall dissociation study (16-brand panel across three tradition
cells; full Japanese cell collapse at Phase B). Brand-format report companion
to the v0.17 SSRN paper (SSRN forthcoming).

Forked from build_report_v16.py with surgical changes:
  - Imports v17_kitchenware_content instead of v16_kitchen_knives_content
  - Section label remains "FINDING"
  - HERO_FIGURE_CAPTIONS replaced with v0.17 brand-format figure captions (3)
  - _slot_lookup table updated for the 3 v0.17 brand-format chart filenames
    (all produced by build_charts_v17.py — single chart-build script for v0.17
    ships both SSRN paper figures and brand-format report figures)
  - Slot name reshuffle to match v17 content pattern slots:
      f1_regime4_canonical -> f1_mention_rates
      f2_per_category      -> f2_cell_collapse
      f3_discourse_language -> f3_dissociation
      f4_per_brand and f5_per_tradition removed (v0.17 has 3 findings)
  - CHART_FIGSIZE_IN extended with v0.17 figsize keys (3 entries)
  - Header right text, citation, output filename, doc metadata updated
  - REPORT_PROTOCOL_VERSION updated to v1.4
  - build_leaderboards_spread returns [] (v0.17, like v0.16, has no
    front-of-report hero spread; the three findings carry the narrative
    directly after the exec summary)
  - Page count target band recalibrated: v0.17 has 3 findings vs v0.16's 5,
    so 8-14 pages instead of 9-20.

Inputs:
  - brand/third_system_brand.json        (v1.5)
  - brand/design_tokens_template.json    (IDML extract)
  - papers/v0_17/figures/chart_v17_*.pdf (3 brand-format charts; all from
                                          build_charts_v17.py)

Output:
  - osf/v17/reports/v17_kitchenware.pdf

Run for v0.17:
    python3 ~/aias/scripts/build_charts_v17.py
    python3 ~/aias/reports/build_report_v17.py

Locked rules followed (same as v0.6 - v0.16):
...
# Make `import v17_kitchenware_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v17_kitchenware_content as content  # noqa: E402
...
CHART_FIGSIZE_IN = {
    ...
    # v0.17 brand-format report keys — 3 charts at 6-col width.
    # All produced by build_charts_v17.py (same script ships SSRN paper + report charts).
    # Figsizes match build_charts_v17.py exactly so charts embed at native size.
    "6_col_v17_mention_rates":  (7.50, 9.00),  # chart_v17_phase_b_mention_rates.pdf — 15-brand horizontal bar
    "6_col_v17_cell_collapse":  (7.50, 5.40),  # chart_v17_cell_collapse.pdf — paired bar chart per cell
    "6_col_v17_dissociation":   (7.50, 7.50),  # chart_v17_dissociation.pdf — scatter with Iwachu callout
}
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Recognition x Recall Dissociation \u00b7 v0.17 \u00b7 May 2026")
...
# Brand-format report figure captions for v0.17 charts.
HERO_FIGURE_CAPTIONS = {
    "f1_mention_rates": (
        "Figure 1 \u00b7 Phase B mention-rate distribution across the v0.17 "
        "operational panel. Each bar is a brand; mention rate is the proportion "
        "of 3 (category queries) \u00d7 6 (reference LLMs) = 18 measurement cells "
        "in which the brand was mentioned. Sorted descending; colored by tradition "
        "cell. The European cell saturates the high-mention end (Le Creuset, "
        "Mauviel, Staub at or near 1.0). The American cell occupies the mid-range "
        "(Lodge, de Buyer, Hestan in the 0.22\u20130.39 band), with two long-tail "
        "brands (Field Company, Smithey) excluded at zero mentions. The Japanese "
        "cell sits entirely at zero \u2014 the full-cell collapse that motivates "
        "the AMBIGUOUS Identity-Load verdict and the v0.18 forward action."
    ),
    "f2_cell_collapse": (
        "Figure 2 \u00b7 Per-cell brand survival from pre-registered panel "
        "(pre-Phase A) to operational panel (post-Phase B). Worldwide n dropped "
        "from 16 to 10 across two attrition stages \u2014 Vermicular at Phase A "
        "(C_P FAIL at 4/6); Field Company, Smithey, Iwachu, Sori Yanagi, Noda Horo "
        "at Phase B. The deficit is 2 below the pre-registered C1 floor of 12. The "
        "European cell remained intact at 6/6; the American cell lost two long-"
        "tail brands; the Japanese cell collapsed entirely. This pattern produces "
        "the FALSIFIED-on-panel-inadequacy verdict for H_Regime4_kitchenware per "
        "the pre-reg decision rule \u00ac C1 \u2228 \u00ac C2."
    ),
    "f3_dissociation": (
        "Figure 3 \u00b7 Recognition \u00d7 Recall dissociation. Phase A C_P "
        "anchoring score (x-axis, out of 6 reference LLMs) versus Phase B "
        "mention rate (y-axis, mentions / 18 cells) for the four pivot brands "
        "that received Phase A measurement. Le Creuset and All-Clad occupy the "
        "full-Presence quadrant (recognition and recall both saturated). "
        "Vermicular failed Phase A (C_P at 4/6) and was descoped before Phase B. "
        "Iwachu sits alone in the recognition-only quadrant: 6/6 Phase A "
        "anchoring (the LLM panel uniformly identifies Iwachu as a Japanese "
        "cookware brand) but 0/18 Phase B mention rate (no LLM surfaces Iwachu "
        "in unprompted premium-cookware category retrieval). The dissociation "
        "motivates the v1.4 Methodology revision specifying AI Availability as "
        "a multi-component construct comprising Recognition and Recall."
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.17 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # All 3 charts from build_charts_v17.py output (single script ships SSRN + report charts):
        "f1_mention_rates": ("chart_v17_phase_b_mention_rates.pdf", "6_col_v17_mention_rates"),
        "f2_cell_collapse": ("chart_v17_cell_collapse.pdf",         "6_col_v17_cell_collapse"),
        "f3_dissociation":  ("chart_v17_dissociation.pdf",          "6_col_v17_dissociation"),
    }
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    citation_text = (
        ...
        "<i>Panel Inadequacy and the Recognition x Recall Dissociation on the "
        "Premium Kitchenware Substrate: AIAS v0.17</i>. "
        "Third System. thirdsystem.ai/v17-kitchenware-dissociation (SSRN forthcoming)"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v17] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v17] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V17_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v17"
    # Charts live in papers/v0_17/figures/ (matches build_charts_v17.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "papers" / "v0_17" / "figures")
    output_path = output_path or (V17_DEPOSIT_ROOT / "reports" / "v17_kitchenware.pdf")
    base_pdf = output_path.parent / "_v17_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v17] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
    ...
    if chart_dir.exists():
        actual_charts = sorted(p.name for p in chart_dir.iterdir()
                                ...
                                and p.name.startswith("chart_v17"))
        unexpected = [n for n in actual_charts if n not in expected_files]
        if unexpected:
            print(f"[build_report_v17] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="Recognition x Recall Dissociation \u2014 AIAS v0.17 Premium Kitchenware",
    ...
    doc.build(story)
    print(f"[build_report_v17] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v17] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v17] page count: {final_pages}")
    if final_pages < 8 or final_pages > 16:
        warnings.warn(
            f"Page count {final_pages} is outside the 8-16 target band."
        )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v17/reports/v17_kitchenware.pdf")
    args = ap.parse_args()
    build(
