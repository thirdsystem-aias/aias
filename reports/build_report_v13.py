#!/usr/bin/env python3
"""
v0.13 Four Empirical Regimes — typesetting pipeline.

Five-category construct-validity expansion (PM Software + Running Shoes + Olive Oil +
Skincare + Personal Finance). Brand-format report companion to the v0.13 SSRN paper
(SSRN 6750498).

Forked from build_report_v12.py with surgical changes:
  - Imports v13_fourregimes_content instead of v12_threeregimes_content
  - Section label remains "FINDING"
  - HERO_FIGURE_CAPTIONS replaced with v0.13 brand-format figure captions
  - _slot_lookup table updated for the 4 v0.13 brand-format chart filenames
    (all produced by build_charts_v13.py — single chart-build script for v0.13
    ships both SSRN paper figures and brand-format report figures)
  - CHART_FIGSIZE_IN extended with v0.13 figsize keys
  - Header right text, citation, output filename, doc metadata updated
  - REPORT_PROTOCOL_VERSION stays at v1.1
  - build_leaderboards_spread returns [] (v0.13, like v0.10/v0.11/v0.12, has no
...
Inputs:
  ...
  - osf/v13/figures/chart_v13_*.pdf      (4 brand-format charts; all from
                                          build_charts_v13.py)

Output:
  - osf/v13/reports/v13_fourregimes.pdf

Run for v0.13:
    python3 build_charts_v13.py
    python3 build_report_v13.py
...
# Make `import v13_fourregimes_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v13_fourregimes_content as content  # noqa: E402
...
# Chart figsize spec (inches). v0.13 brand-format report uses 4 charts at 6-col
# width, with varied heights matching the figsize used in build_charts_v13.py
# (single chart-build script for v0.13 ships both SSRN paper and report figures).
CHART_FIGSIZE_IN = {
    ...
    # v0.12 keys retained for cross-version cross-reference (not used by v0.13 report)
    ...
    # v0.13 brand-format report keys — 4 charts at 6-col width.
    # All produced by build_charts_v13.py (same script ships SSRN paper + report charts).
    "6_col_v13_fourregime":       (7.50, 5.50),  # chart_v13_h7_fourregime_classification.pdf
    "6_col_v13_mint_phantom":     (7.50, 5.50),  # chart_v13_h8_mint_phantom.pdf
    "6_col_v13_per_category_rho": (7.50, 5.00),  # chart_v13_per_category_rho_comparison.pdf
    "6_col_v13_pooled_scatter":   (7.50, 5.50),  # chart_v13_pooled_rank_scatter.pdf
}
...
class V13DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Four Empirical Regimes \u00b7 v0.13 \u00b7 12 May 2026")
...
def content_meta_line(doc: V13DocTemplate) -> str:
...
# Brand-format report figure captions for v0.13 charts.
HERO_FIGURE_CAPTIONS = {
    "f1_fourregime_classification": (
        "Figure 1 \u00b7 Four empirical regimes at v0.13. Each category at bivariate "
        "<font name='Helvetica'>\u03c1</font> \u00d7 covariate decrement. Three regimes pre-registered "
        "(background bands); skincare and finance fall outside all three, defining a provisional "
        "fourth pattern (Covariate-saturated weak). Olive oil routes to Regime 3 descriptive-only "
        "per pre-reg \u00a73.6a (n = 8, below n = 10 hard floor) and is not plotted. Points coloured "
        "by H7 regime classification."
    ),
    "f2_mint_phantom": (
        "Figure 2 \u00b7 Mint phantom-persistence canonical confirmation. Mint retains 44.79% / "
        "41.67% AI Presence across the two May 2026 waves and ranks fifth in personal finance at "
        "both \u2014 with zero Trends signal in both regions. All three H8 conditions hold: C1 (AI "
        "\u2265 5% both waves), C2 (AI rank 5 / 5), C3 (not Trends top-5 either wave, trivially "
        "satisfied per \u00a711 since Mint is E1b-ineligible). The cleanest phantom-persistence anchor "
        "the programme has produced."
    ),
    "f3_per_category_rho": (
        "Figure 3 \u00b7 Per-category Spearman <font name='Helvetica'>\u03c1</font> at t<sub size='6'>1</sub> "
        "and t<sub size='6'>2</sub>, four confirmatory categories. Running anchors the upper end "
        "(<font name='Helvetica'>\u03c1</font> \u2248 0.80); PM software at the marginal-direct boundary "
        "(\u2248 0.50); skincare at 0.28\u20130.33 (boundary-flagged below Regime 1); finance at "
        "0.09\u20130.17 (well into Regime 4). Olive oil descriptive-only per the n-floor route, "
        "annotated on the right edge."
    ),
    "f4_pooled_scatter": (
        "Figure 4 \u00b7 Pooled rank-within-category alignment across the four confirmatory "
        "categories at both waves. Despite the per-category heterogeneity in Figure 3, the "
        "aggregate signal is moderately strong and stable: pooled <font name='Helvetica'>\u03c1</font> "
        "= 0.459 at t<sub size='6'>1</sub> and 0.475 at t<sub size='6'>2</sub> (n \u2248 70, p < 0.0001 at "
        "both waves). AI Presence and Trends rank-orders co-vary in aggregate even where the "
        "within-category relationship varies by regime."
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.13 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # All 4 charts from build_charts_v13.py output (single script ships SSRN + report charts):
        "f1_fourregime_classification": ("chart_v13_h7_fourregime_classification.pdf", "6_col_v13_fourregime"),
        "f2_mint_phantom":              ("chart_v13_h8_mint_phantom.pdf",              "6_col_v13_mint_phantom"),
        "f3_per_category_rho":          ("chart_v13_per_category_rho_comparison.pdf",  "6_col_v13_per_category_rho"),
        "f4_pooled_scatter":            ("chart_v13_pooled_rank_scatter.pdf",          "6_col_v13_pooled_scatter"),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    """v0.13 patterns all render on the SPREAD template (full-width frame),
    treated as hero (chart embedded inline in section's text flow)."""
    slot = pattern["chart_slot"]
    is_hero = slot in (
        "f1_fourregime_classification", "f2_mint_phantom",
        "f3_per_category_rho", "f4_pooled_scatter",
    )
    ...
    if chart_path is not None and paragraphs:
        ...
        if is_hero:
            ...
            n_lead = max(0, min(n_lead, n_paragraphs))
            lead = paragraphs[:n_lead]
...
def build_hypothesis_scoring_story(styles: dict) -> list:
    ...
    # v0.13 has 12 rows (more than v0.11's 6). Slightly tighter cell padding
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    citation_text = (
        ...
        "Construct-Validity Expansion of the v0.12 Three-Empirical-Regimes Finding "
        "(PM Software, Running Shoes, Olive Oil, Skincare, Personal Finance) \u2014 "
        "AI Presence Index v0.13</i>. "
        "Third System. thirdsystem.ai/v13-four-empirical-regimes (SSRN 6750498)"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v13] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v13] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V13_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v13"
    chart_dir = chart_dir or (V13_DEPOSIT_ROOT / "figures")
    output_path = output_path or (V13_DEPOSIT_ROOT / "reports" / "v13_fourregimes.pdf")
    base_pdf = output_path.parent / "_v13_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v13] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "f1_fourregime_classification", "f2_mint_phantom",
        "f3_per_category_rho", "f4_pooled_scatter",
    ]
    ...
    if chart_dir.exists():
        actual_charts = sorted(p.name for p in chart_dir.iterdir()
                                ...
                                and p.name.startswith("chart_v13"))
        unexpected = [n for n in actual_charts if n not in expected_files]
        if unexpected:
            print(f"[build_report_v13] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V13DocTemplate(
        ...
        title="Four Empirical Regimes \u2014 AI Presence Index v0.13",
    ...
    # --- (No leaderboards spread for v0.13, like v0.10/v0.11.) ---
    ...
    doc.build(story)
    print(f"[build_report_v13] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v13] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v13] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v13/reports/v13_fourregimes.pdf")
    args = ap.parse_args()
    build(
