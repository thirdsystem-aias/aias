#!/usr/bin/env python3
"""
v0.20 Skincare — typesetting pipeline.

Type 2 emergence phase (n=24, three IL-gradient cells: Prestige / Celebrity DTC /
Clinical). Recognition × Recall dissociation construct extends to a fourth
substrate family with Iwachu cases in all three cells; v1.5's Type 2 quadrant
populated empirically for the first time (Glossier and Rhode in Cell B);
v1.5 C2 multi-statistic prospectively tested across three differently-shaped
cells (Cell C saturated → reject; Cell A bimodal → reject; Cell B
variance-rich → pass). First prospective phase under v1.5 methodology.
Brand-format report companion to the v0.20 SSRN paper (SSRN 6811441).

Forked from build_report_v19.py with surgical changes:
  - Imports v20_skincare_content instead of v19_audiophile_content
  - Section label remains "FINDING"
  - HERO_FIGURE_CAPTIONS replaced with v0.20 brand-format figure captions (3)
  - _slot_lookup table updated for the 3 v0.20 brand-format chart filenames
    (all produced by build_charts_v20.py at reports/figs/v20/):
  ...
  - CHART_FIGSIZE_IN extended with v0.20 figsize keys (3 entries at 7.5x5.5,
    matching the native figsize used in build_charts_v20.py)
  - Header right text updated: "Type 2 Hunt · v0.20 · May 2026"
  - Citation updated: v0.20 skincare, SSRN 6811441
  - Output filename: v20_skincare.pdf
  - REPORT_PROTOCOL_VERSION updated to v1.5 (first prospective v1.5 phase)
  - Chart directory: reports/figs/v20/
  - Page count target band: 8-14 pages (v0.20 has 3 findings, same as v0.19)

Inputs:
  - brand/third_system_brand.json        (v1.5)
  - brand/design_tokens_template.json    (IDML extract)
  - reports/figs/v20/chart_01_*.pdf      (3 brand-format charts; all from
                                          build_charts_v20.py)

Output:
  - osf/v20/reports/v20_skincare.pdf

Run for v0.20:
    python3 ~/aias/scripts/build_charts_v20.py
    python3 ~/aias/reports/build_report_v20.py
...
# Make `import v20_skincare_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v20_skincare_content as content  # noqa: E402
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.setFillColor(HexColor(self.palette.soft_black))
        # v0.20 header right text — updated from v0.19
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Type 2 Hunt \u00b7 v0.20 \u00b7 May 2026")
...
# v0.20 brand-format report figure captions for the three v0.20 charts.
HERO_FIGURE_CAPTIONS = {
    "f1_cp_distribution": (
        "Figure 1 \u00b7 Phase A Recognition (C_P) score per brand, per cell. "
        "Each point is one brand; jitter spreads points within cells for "
        "visibility. Cell A (Prestige) is bimodal: six brands at C_P = 6/6 "
        "(Est\u00e9e Lauder, Lanc\u00f4me, La Mer, Sisley, La Prairie, Cl\u00e9 "
        "de Peau Beaut\u00e9) and two at C_P = 3/6 (Dior Beauty, Chanel "
        "Beauty) \u2014 distinct count = 2, modal share = 0.750; v1.5 C2 "
        "rejects. Cell C (Clinical) is fully saturated: all eight brands at "
        "C_P = 6/6 \u2014 distinct count = 1, modal share = 1.000; v1.5 C2 "
        "rejects. Cell B (Celebrity DTC) is variance-rich: five at 6/6 "
        "(Rhode, Fenty Skin, Drunk Elephant, Glossier, Augustinus Bader), "
        "two at 2/6 (Goop, Tower 28), one at 0/6 (Rare Beauty) \u2014 "
        "distinct count = 3, modal share = 0.625; v1.5 C2 passes. The "
        "first prospective v1.5 C2 exercise behaves on three differently-"
        "shaped cells exactly as the methodology paper specified."
    ),
    "f2_dissociation_scatter": (
        "Figure 2 \u00b7 Recognition \u00d7 Recall dissociation scatter \u2014 "
        "cumulative anchor base across four substrate families. Each "
        "colored point is one v0.20 skincare brand: Phase A C_P (x-axis, "
        "0\u20136) versus Phase B category-anchored mention count (y-axis, "
        "0\u201318). Gray ghost markers show v0.17 Japanese-cell, v0.18 "
        "indie-fragrance, and v0.19 audiophile Iwachu cases. Shaded "
        "quadrant: Iwachu-pattern (C_P \u2265 5 \u2227 mentions \u2264 2). "
        "v0.20 contributes 11 cases distributed across all three cells \u2014 "
        "the first substrate to populate every IL tier. The AIAS\u2122 "
        "v1.4/v1.5 multi-component construct now spans four substrate "
        "families with multi-cell distribution."
    ),
    "f3_channel_asymmetry": (
        ...
        "\u2227 R_cult \u2265 5). One Type 1 case in v0.20: SkinCeuticals "
        "(R_cat = 12, R_cult = 4) in Cell C. Two clean Type 2 cases: "
        "Glossier (R_cat = 0, R_cult = 9) and Rhode (R_cat = 0, R_cult = "
        "8), both Cell B; Augustinus Bader (R_cat = 2, R_cult = 7) sits "
        "at the threshold boundary. The Type 2 quadrant, predicted but "
        "unpopulated across v0.17\u2013v0.19, is now empirically anchored."
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.20 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # All 3 charts from build_charts_v20.py at reports/figs/v20/.
        "f1_cp_distribution": (
            "chart_01_cp_distribution.pdf",
            "6_col_v20_cp_distribution",
        ),
        "f2_dissociation_scatter": (
            "chart_02_dissociation_scatter.pdf",
            "6_col_v20_dissociation_scatter",
        ),
        "f3_channel_asymmetry": (
            "chart_03_channel_asymmetry.pdf",
            "6_col_v20_channel_asymmetry",
        ),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    if chart_path is not None and figsize_key is not None:
        ...
        # v0.20 hero slot names — same as v0.18/v0.19 (3 findings pattern)
        is_hero = slot in (
            "f1_cp_distribution",
            "f2_dissociation_scatter",
            "f3_channel_asymmetry",
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.20 protocol version — first prospective v1.5 phase
    ...
    citation_text = (
        ...
        "<i>Type 2 Emergence and First Prospective v1.5 C2 Calibration on a "
        "Skincare IL-Gradient Substrate: AIAS v0.20</i>. "
        "Third System. thirdsystem.ai/v20-skincare (SSRN 6811441)"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v20] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v20] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V20_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v20"
    # Charts live in reports/figs/v20/ (matches build_charts_v20.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v20")
    output_path = output_path or (V20_DEPOSIT_ROOT / "reports" / "v20_skincare.pdf")
    base_pdf = output_path.parent / "_v20_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v20] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v20] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="Type 2 Hunt \u2014 AIAS v0.20 Skincare",
    ...
    doc.build(story)
    print(f"[build_report_v20] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v20] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v20] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v20/reports/v20_skincare.pdf")
    args = ap.parse_args()
    build(
