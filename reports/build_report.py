#!/usr/bin/env python3
"""
Phase C — Cross-Category Findings v0.6 typesetting pipeline.
...
Inputs:
  ...
  - brand/v06_source_text.md             (body copy, extracted from v0.6 PDF)
  - reports/output/chart_*.pdf           (9 vector charts from Phase B)
  - brand/THIRDSYSTEM_Logo.svg           (running header wordmark)
  - brand/THIRDSYSTEM_AIPT_Logo.svg      (cover product lockup)

Output:
  - reports/output/v06_cross_category.pdf

Approach (two-pass):
  Pass 1: ReportLab builds the body layout. At each chart slot, it leaves a blank
          reservation of the locked figsize and records (page, x, y, w, h) in a
          manifest. The base PDF has typography, footer, header, etc. but no charts.
  Pass 2: pypdf merges each chart PDF onto the corresponding reserved rectangle
          on the corresponding page. Vector quality is preserved end-to-end.
...
Locked rules followed:
  ...
  - Citation, methodology disclaimer, public-report footer used verbatim from
    third_system_brand.json copy_boilerplate equivalents.
  - Samsung is OMITTED from byline (commercial output rule).
"""
...
import v06_content as content  # noqa: E402
...
class V06DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Cross-Category Findings · v0.6 · April 2026")
...
def build_leaderboards_spread(styles: dict, manifest: ChartManifest,
    ...
    s.append(KeepTogether([
        Paragraph("Cross-category leaderboards", styles["h1"]),
        Paragraph(
            "Top-five Presence scores per category. The leaderboards are the factual "
            "base on which the six patterns are reasoned.",
            styles["body_lead"]),
    ]))
    s.append(Spacer(1, 8))
    s.append(ChartReservation(
        ...
        debug=debug,
        caption=("Figure A \u00b7 Top-five brands by Presence in each of the five measured "
                 "categories. The within-category spread between AI-mediated leaders and "
                 "category incumbents is the structural surface this report analyzes."),
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    is_hero = slot in ("hero_p2_scatter", "hero_p4_country")
    ...
    s.append(KeepTogether([
        Spacer(1, 4),
        Paragraph(f"PATTERN {pattern['number']}", styles["pattern_number"]),
...
HERO_FIGURE_CAPTIONS = {
    "hero_p2_scatter": (
        "Figure 2 \u00b7 Comparison prompts converge on incumbents; Discovery prompts "
        "elevate a sharply different set. The asymmetry repeats across all five "
        "categories \u2014 sharpest in PM software, running shoes, and olive oil."
    ),
    "hero_p4_country": (
        "Figure 4 \u00b7 Country-of-origin two-panel: olive oil (left) and premium "
        "skincare (right). Spanish brands under-perform in olive oil; Korean brands "
        "are absent from skincare \u2014 in both cases, English-language discourse "
        "dominates the AI training corpus."
    ),
}
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    ...
    table = {
        "inline_p1_variance":             ("chart_p1_3col.pdf",          "3_col_inline"),
        "hero_p2_scatter":                ("chart_p2_6col.pdf",          "6_col_hero"),
        "inline_p3_awareness_gap":        ("chart_p3_4col.pdf",          "3_col_inline"),
        "hero_p4_country":                ("chart_p4_spread.pdf",        "spread"),
        "inline_p5_default_reinforcement":("chart_p5_3col.pdf",          "3_col_inline"),
        "inline_p6_phantom":              ("chart_p6_3col.pdf",          "3_col_inline"),
        "hero_leaderboards":              ("chart_leaderboards_6col.pdf","6_col_hero_xl"),
        "hero_aggregate":                 ("chart_aggregate_6col.pdf",   "6_col_hero_tall"),
    }
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    citation_text = (
        ...
        "<i>Cross-Category Findings: AI Presence Index v0.6</i>. "
        "Third System. thirdsystem.ai/v06-cross-category"
    )
...
def build(*, debug_layout: bool = False,
    ...
    """
    Build the v0.6 cross-category report.
    ...
    print(f"[build_report] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    # Preconditions
    chart_dir = chart_dir or OUTPUT_DIR
    output_path = output_path or (OUTPUT_DIR / "v06_cross_category.pdf")
    base_pdf = OUTPUT_DIR / "_v06_base.pdf"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # Pre-flight: report which charts are present and which are missing.
    print(f"[build_report] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "inline_p1_variance", "hero_p2_scatter", "inline_p3_awareness_gap",
        "hero_p4_country", "inline_p5_default_reinforcement", "inline_p6_phantom",
        "hero_leaderboards", "hero_aggregate",
    ]
    expected_files = set()
    ...
    if chart_dir.exists():
        ...
        if actual_charts:
            ...
            if unexpected:
                print(f"[build_report] chart files in dir not matched by any slot:")
                for n in unexpected:
                    print(f"  [UNUSED ] {n}")
    ...
    doc = V06DocTemplate(
        ...
        title="Cross-Category Findings — AI Presence Index v0.6",
    ...
    # --- Patterns 1-6 ---
    # ALL patterns render on the SPREAD template (single wide frame with
    # BalancedColumns(2) providing the column split). No template switching
    # between patterns, no forced PageBreaks — patterns flow continuously.
    #
    # Layout strategy (see build_pattern_unified):
    # - Non-hero patterns: chart inside BC at top of column 1, body wraps.
    # - Hero patterns: SANDWICH — lead body in BC, then full-width chart
    #   spanning both columns, then rest of body in BC. Magazine-style
    #   figure embedding within the section's text flow.
    #
    # Pattern transitions: when one pattern ends, the next pattern's title
    # KeepTogether group lands wherever space allows on the same page; if
    # not enough room, ReportLab pushes the title to the next page.
    story.append(NextPageTemplate("spread"))
    story.append(PageBreak())
    ...
    # --- Aggregate matrix hero (placement Q3 default: closing summary) ---
    aggregate_flow = build_aggregate_spread(styles, manifest, chart_dir, debug_layout)
    if aggregate_flow:
        story.append(NextPageTemplate("spread"))
        story.append(PageBreak())
        story.extend(aggregate_flow)
    ...
    # Build base PDF
    doc.build(story)
    print(f"[build_report] base PDF written: {base_pdf} "
    ...
    print(f"[build_report] FINAL PDF written: {output_path}")

    # Page count check
    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ./output/v06_cross_category.pdf")
    args = ap.parse_args()
    build(
