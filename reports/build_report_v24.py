#!/usr/bin/env python3
"""
v0.24 Automotive — typesetting pipeline.
...
Brand-format report companion to the v0.24 SSRN paper (SSRN TBD).

Forked from build_report_v21.py with surgical changes:
  - Imports v24_automotive_content instead of v21_cosmetics_content
  - HERO_FIGURE_CAPTIONS rewritten as v0.24 templates (4 entries; the
    fourth — f4_phantom_defunct — is net new for v0.24)
  - _slot_lookup table updated for the 4 v0.24 brand-format chart filenames
    (all produced by build_charts_v24.py at reports/figs/v24/):
  ...
  - CHART_FIGSIZE_IN extended with v0.24 figsize keys (4 entries; v0.21
    entries preserved for cross-version reuse)
  - Header right text updated: "Phantom Brand Persistence · v0.24 · May 2026"
  - Citation updated: v0.24 automotive, SSRN TBD
  - Output filename: v24_premium_spirits.pdf
  - REPORT_PROTOCOL_VERSION: v1.5 → v1.6 (first prospective v1.6 phase)
  - Chart directory: reports/figs/v24/
  - Deposit root: osf/v24/
  - Page count target band: 8-14 pages (v0.24 has 4 findings vs v0.21's 3;
...
Inputs:
  ...
  - reports/figs/v24/chart_01_*.pdf      (4 brand-format charts; all from
                                          build_charts_v24.py)

Output:
  - osf/v24/reports/v24_premium_spirits.pdf

Run for v0.24:
    python3 ~/aias/scripts/build_charts_v24.py
    python3 ~/aias/reports/build_report_v24.py
...
# Make `import v24_premium_spirits_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v24_premium_spirits_content as content  # noqa: E402
...
CHART_FIGSIZE_IN = {
    ...
    # v0.24 brand-format report keys — 4 charts at native figsize 7.5 × 5.5.
    # Matches build_charts_v24.py figure sizes; chart_04 is net new for v0.24.
    "6_col_v24_cp_distribution":      (7.50, 5.50),
    "6_col_v24_dissociation_scatter": (7.50, 5.50),
    "6_col_v24_channel_asymmetry":    (7.50, 5.50),
    "6_col_v24_phantom_defunct":      (7.50, 5.50),
}
...
class ChartReservation(Flowable):
    def __init__(self, slot_key: str, chart_path: Path,
        ...
        # v0.24: auto-size reservation height to match the actual chart PDF's
        # aspect ratio. bbox_inches='tight' in build_charts_v24 produces
    ...
    def wrap(self, available_w, available_h):
        ...
        if self.caption_p is not None:
            ...
            total_h = self.chart_h_pt + ch + 2   # was + 4 (v0.24 tightening)
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename: str, *,
        ...
        # v0.24 lead-page layout — REVERTED to v0.21 / aias_1_0 working pattern:
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.setFillColor(HexColor(self.palette.soft_black))
        # v0.24 header right text
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Phantom Brand Persistence \u00b7 v0.24 \u00b7 May 2026")
...
# v0.24 brand-format report figure captions. Templates contain locked structural
...
# (Step 4 of the v0.24 workflow, after measurement yields verdicts).
HERO_FIGURE_CAPTIONS = {
    "f1_cp_distribution": (
        "Figure 1 \u00b7 Phase A Recognition (C_P) score per brand, per cell. "
        "Each point is one brand; the automotive substrate is tested for the "
        "v1.6 substrate Recognition pre-screen (Inc1) outcome \u2014 uniform "
        "saturation across cells (predicted) vs differential saturation. "
        "Cell A_Heritage (n=7): Mercedes-Benz, Jaguar, Cadillac, Rolls-Royce, "
        "Bentley, Porsche, BMW. Cell B_Disruptor (n=5): Tesla, Rivian, Lucid, "
        "Polestar, Fisker. Cell C_Mass-Legacy (n=7): Toyota, Honda, Ford, "
        "Chevrolet, Hyundai, Volkswagen, Nissan. Cell D_Defunct (n=5): "
        "Pontiac, Oldsmobile, Plymouth, Mercury, Saturn \u2014 discontinued "
        "corporate brands tested as a pure-phantom panel."
        "  [TBD post-acquisition: per-cell mean C_P, distinct count, modal "
        "share, v1.5 C2 status.]"
    ),
    "f2_dissociation_scatter": (
        ...
        "point is one v0.24 automotive brand: Phase A C_P (x-axis, 0\u20136) "
        "versus Phase B category-anchored mention count (y-axis, 0\u201318). "
        "Shaded quadrant: Iwachu-pattern (C_P \u2265 5 \u2227 R_cat \u2264 2). "
        "v0.24 contributes the 6th substrate family; the AIAS\u2122 v1.4/v1.5 "
    ...
    "f3_channel_asymmetry": (
        ...
        "\u00d7 6 models = max 18). For the v0.24 automotive panel, the "
        ...
        "Type 1 (canonical-preferred; R_cat \u2265 5 \u2227 R_cult \u2264 2). "
        "Upper-left quadrant: Type 2 (cultural-preferred; R_cat \u2264 2 "
        "\u2227 R_cult \u2265 5). Cell A_Heritage R_phantom is the supporting "
        "test for H_Phantom_Brand_Persistence_heritage; threshold ladder "
        "\u2265 8 CONFIRMED / 4\u20137 PARTIAL / < 4 FALSIFIED. "
        "[TBD post-acquisition: Type 1 / Type 2 case lists and verdicts.]"
    ),
    "f4_phantom_defunct": (
        "Figure 4 \u00b7 Phantom Brand Persistence \u2014 defunct corporate "
        ...
        "CONFIRMED threshold (N \u2265 3, locked per v0.24-prereg-r1) render "
        ...
        "pure-phantom upper-bound test for AIAS\u2122 Phantom Brand "
        "Persistence \u2014 the first measurement of whether the AI mediation "
        "layer surfaces documentably dead corporate brands as if currently "
        "active. No prior AIAS substrate (kitchen knives, kitchenware, indie "
        "fragrance, audiophile headphones, skincare, cosmetics) supports this "
        "test; automotive is uniquely positioned because corporate-brand "
        "death is documented and dated. "
        "[TBD post-acquisition: H_Phantom_Defunct verdict and per-brand "
        "R_phantom_defunct counts.]"
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.24 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # All 4 charts from build_charts_v24.py at reports/figs/v24/.
        "f1_cp_distribution": (
            "chart_23_composite_bar.pdf",
            "6_col_v24_cp_distribution",
        ),
        "f2_dissociation_scatter": (
            "chart_23_recall_scatter.pdf",
            "6_col_v24_dissociation_scatter",
        ),
        "f3_channel_asymmetry": (
            "chart_23_channel_heatmap.pdf",
            "6_col_v24_channel_asymmetry",
        ),
        "f4_phantom_defunct": (
            "chart_23_conglomerate_box.pdf",
            "6_col_v24_phantom_defunct",
        ),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    if chart_path is not None and figsize_key is not None:
        ...
        # v0.24 hero slot names — 4 findings pattern (one more than v0.18-v0.21)
        is_hero = slot in (
            "f1_cp_distribution",
            "f2_dissociation_scatter",
            "f3_channel_asymmetry",
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.24 protocol version — v1.6 (Phantom Brand Persistence increment Inc3)
    ...
    citation_text = (
        ...
        "Substrate: AIAS v0.24</i>. "
        "Third System. thirdsystem.ai/v24-automotive (SSRN TBD)"
    )
...
def overlay_charts(base_pdf_path: Path, manifest: ChartManifest,
    ...
    for slot in manifest.slots:
        ...
        # v0.24: scale = min(sx, sy) — was * 0.92 (8% safety margin). The
        ...
        # matched to build_charts_v24.py figsize, the margin is unnecessary.
        scale = min(sx, sy)
        offset_x = slot.x_pt + (target_w - chart_w * scale) / 2

        # v0.24: bullet-proof bottom-whitespace auto-crop. Detect any blank
        ...
        # bbox_inches='tight' in build_charts_v24.py will have ~0pt blank
        ...
        bottom_blank_pt = _measure_chart_bottom_whitespace_pt(
            slot.chart_path, chart_h)
        bottom_blank_scaled = bottom_blank_pt * scale

        if slot.h_pt > 350.0:
            # Top-aligned: shift up by detected bottom-blank, so visible
            # content sits flush at the top of the reservation.
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v24] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v24] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v24"
    # Charts live in reports/figs/v24/ (matches build_charts_v24.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v24")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v24_premium_spirits.pdf")
    base_pdf = output_path.parent / "_v24_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v24] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v24] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="Phantom Brand Persistence \u2014 AIAS v0.24 Automotive",
    ...
    doc.build(story)
    print(f"[build_report_v24] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v24] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v24] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v24/reports/v24_premium_spirits.pdf")
    args = ap.parse_args()
    build(
