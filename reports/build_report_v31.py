...
# Make `import v31_cpc_baseline_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v31_cpc_baseline_content as content  # noqa: E402
...
# v0.31: PATTERNS is a synthesis prose string; the chart-bearing findings ARE the
...
# HYPOTHESIS_SCORING (v0.31): [{id, proposition, verdict, basis}] → {heading, intro, rows}
if isinstance(content.HYPOTHESIS_SCORING, list) and content.HYPOTHESIS_SCORING and isinstance(content.HYPOTHESIS_SCORING[0], dict):
    _STATUS_CLASS = {"Established": "confirmed", "Confirmed": "confirmed",
                     "Documented": "confirmed", "By design": "descriptive"}
    _rows = []
    for h in content.HYPOTHESIS_SCORING:
        _vt = h.get("verdict", "")
        _rows.append((h["id"], h.get("proposition", ""), _vt, _STATUS_CLASS.get(_vt, "partial")))
...
# HYPOTHESIS_DETAILS (v0.31 dict P1-P5) is consumed into the P-findings above;
...
if isinstance(content.CLOSING, str):
    content.CLOSING = {
        ...
        "datasets": [
            "Per-brand CPC: osf.io/ec6wh/v31/data/v31_cpc.csv",
            "Verdicts: osf.io/ec6wh/v31/v31_cpc_verdicts.json",
            "Scoring code: osf.io/ec6wh/v31/scripts/score_v0_31.py",
        ],
        "methodology_log": "v0.31 (SSRN pending)",
...
if isinstance(content.CLOSING, dict):
    ...
    content.CLOSING.setdefault("methodology_log", "v0.31 (SSRN pending)")
...
CHART_FIGSIZE_IN = {
    ...
    # v0.31 CPC figures — native dims from chart_0*.pdf mediaboxes
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "AIAS\u2122 Consistency (CPC) \u00b7 v0.31 \u00b7 June 2026")
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    ...
    table = {
        "within": ("chart_02_cpc_within_substrate.pdf",   "v31_within"),
        "cross":  ("chart_03_cpc_cross_category.pdf",     "v31_cross"),
        "floor":  ("chart_04_defined_undefined_floor.pdf", "v31_floor"),
        "gate":   ("chart_01_reconciliation_gate.pdf",    "v31_gate"),
    }
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    citation_text = (
        ...
        "AIAS Consistency Component (CPC), v0.31</i>. "
        "Third System (SSRN pending). thirdsystem.ai/v31"
    )
...
def build(*, debug_layout: bool = False,
    ...
    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v31"
    # v0.31 figures live in reports/figs/v31/ (build_charts_v31.py default
    # fig-dir), the same chart_0*.pdf the SSRN paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v31" / "report")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v31_cpc_baseline_report.pdf")
    base_pdf = output_path.parent / "_v31_base.pdf"
    ...
    doc = V15DocTemplate(
        ...
        title="Consistency, Category by Category \u2014 AIAS v0.31",
