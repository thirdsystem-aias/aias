#!/usr/bin/env python3
"""
v0.18 Indie Fragrance — typesetting pipeline.

Recognition x Recall dissociation generalization study (24-brand panel across
three IL-stratified cells: mass-prestige, designer-niche, indie/artisan).
Brand-format report companion to the v0.18 SSRN paper (forthcoming).

Forked from build_report_v17.py with surgical changes:
  - Imports v18_indie_fragrance_content instead of v17_kitchenware_content
  - Section label remains "FINDING"
  - HERO_FIGURE_CAPTIONS replaced with v0.18 brand-format figure captions (3)
  - _slot_lookup table updated for the 3 v0.18 brand-format chart filenames
    (all produced by build_charts_v18.py at reports/figs/v18/):
      f1_mention_rate_distribution -> chart_01_mention_rate_distribution.pdf
      f2_cell_attrition            -> chart_02_cell_attrition.pdf
      f3_dissociation_scatter      -> chart_03_dissociation_scatter.pdf
  - CHART_FIGSIZE_IN extended with v0.18 figsize keys (3 entries at 6.0x4.0,
    matching the native figsize used in build_charts_v18.py)
  - Header right text updated: "Recognition x Recall Generalization · v0.18 · May 2026"
  - Citation updated: v0.18 indie fragrance, SSRN forthcoming
  - Output filename: v18_indie_fragrance.pdf
  - REPORT_PROTOCOL_VERSION updated to v1.4
  - Chart directory: reports/figs/v18/ (v0.18 charts live alongside the build
    script in reports/ rather than papers/v0_18/figures/)
  - Page count target band: 8-14 pages (v0.18 has 3 findings, same as v0.17)

Inputs:
  - brand/third_system_brand.json        (v1.5)
  - brand/design_tokens_template.json    (IDML extract)
  - reports/figs/v18/chart_01_*.pdf      (3 brand-format charts; all from
                                          build_charts_v18.py)

Output:
  - osf/v18/reports/v18_indie_fragrance.pdf

Run for v0.18:
    python3 ~/aias/scripts/build_charts_v18.py
    python3 ~/aias/reports/build_report_v18.py

Locked rules followed (same as v0.6 - v0.17):
...
# Make `import v18_indie_fragrance_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v18_indie_fragrance_content as content  # noqa: E402
...
CHART_FIGSIZE_IN = {
    # v0.11–v0.17 keys preserved for cross-version reuse (not used by v0.18 report)
...
# ---------------------------------------------------------------------------
# Font registration (verbatim from build_report_v17.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Paragraph styles (verbatim from build_report_v17.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Chart slot tracker, reservation Flowable (verbatim from v0.17)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DocTemplate (functionally verbatim from v0.17; only header right text changed)
# ---------------------------------------------------------------------------
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.setFillColor(HexColor(self.palette.soft_black))
        # v0.18 header right text — updated from v0.17
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Recognition x Recall Generalization \u00b7 v0.18 \u00b7 May 2026")
...
# ---------------------------------------------------------------------------
# Story builders (verbatim from v0.17)
# ---------------------------------------------------------------------------
...
# v0.18 brand-format report figure captions for the three v0.18 charts.
HERO_FIGURE_CAPTIONS = {
    "f1_mention_rate_distribution": (
        "Figure 1 \u00b7 Phase B mention-rate distribution per cell. Box-and-whisker "
        "plots show within-cell distribution of brand mention counts across the "
        "3 (category queries) \u00d7 6 (reference LLMs) = 18 measurement cells. "
        "Cell A (designer-niche) shows distributed Recall (Le Labo 17/18, MFK "
        "16/18, Diptyque 15/18 lead a broad coverage). Cell B (indie/artisan) "
        "shows near-total Recall attrition (only D.S. & Durga with non-zero "
        "mentions, at 2/18). Cell C (mass-prestige) surfaces sporadically "
        "(Tom Ford 4, Chanel 3, Versace and Dior 1 each) despite failing the "
        "niche-fragrance Recognition probe \u2014 evidence of a parallel "
        "cultural-footprint Recall channel."
    ),
    "f2_cell_attrition": (
        "Figure 2 \u00b7 Cell attrition from Phase A registered panel to Phase B "
        "mention-positive set. Light bars are the registered panel (n = 8 per "
        "cell); dark bars are brands with \u2265 1 mention across the 18 Phase B "
        "observations. Cell A retains 7 of 8 (only Memo Paris is mention-zero); "
        "Cell B retains 1 of 8 (D.S. & Durga alone); Cell C retains 4 of 8 "
        "(the four with cultural-footprint discourse channels). The cell-"
        "attrition pattern is non-monotonic across IL tiers \u2014 the IL-gradient "
        "operates as dissociation pattern at the high-IL tier rather than as "
        "Pareto concentration."
    ),
    "f3_dissociation_scatter": (
        "Figure 3 \u00b7 Recognition \u00d7 Recall dissociation scatter for the v0.18 "
        "panel. Phase A C_P anchoring score (x-axis, 0\u20136 across the reference "
        "panel) versus Phase B mention count (y-axis, 0\u201318 across the query "
        "battery). Shaded quadrant: Iwachu-pattern (C_P \u2265 5 and mentions "
        "\u2264 2). Black \u00d7 marks the v0.17 Iwachu reference anchor (C_P = 6, "
        "mentions = 0). v0.18 contributes 9 cases to the shaded quadrant: "
        "3 in Cell A (Comme des Gar\u00e7ons Parfums, Memo Paris, Etat Libre "
        "d'Orange) and 6 in Cell B (D.S. & Durga, Boy Smells, Heretic Parfum, "
        "Vyrao, Phlur, Snif). Cell C contributes zero by Recognition-floor "
        "design \u2014 no Cell C brand satisfies C_P \u2265 5. The construct "
        "generalizes from cross-cultural anchor to same-language substrate."
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.18 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # All 3 charts from build_charts_v18.py at reports/figs/v18/.
        "f1_mention_rate_distribution": (
            "chart_01_mention_rate_distribution.pdf",
            "6_col_v18_mention_rate_distribution",
        ),
        "f2_cell_attrition": (
            "chart_02_cell_attrition.pdf",
            "6_col_v18_cell_attrition",
        ),
        "f3_dissociation_scatter": (
            "chart_03_dissociation_scatter.pdf",
            "6_col_v18_dissociation_scatter",
        ),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    if chart_path is not None and figsize_key is not None:
        ...
        # v0.18 hero slot names — update from v0.17
        is_hero = slot in (
            "f1_mention_rate_distribution",
            "f2_cell_attrition",
            "f3_dissociation_scatter",
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.18 protocol version
    REPORT_PROTOCOL_VERSION = "v1.4"
    ...
    citation_text = (
        ...
        "<i>Identity-Load Moderator Test and Recognition x Recall Dissociation "
        "Generalization on an English-Language Indie Fragrance Substrate: "
        "AIAS v0.18</i>. "
        "Third System. thirdsystem.ai/v18-indie-fragrance (SSRN forthcoming)"
    )
...
# ---------------------------------------------------------------------------
# Pass 2 overlay (verbatim from v0.17)
# ---------------------------------------------------------------------------
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v18] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v18] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V18_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v18"
    # Charts live in reports/figs/v18/ (matches build_charts_v18.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v18")
    output_path = output_path or (V18_DEPOSIT_ROOT / "reports" / "v18_indie_fragrance.pdf")
    base_pdf = output_path.parent / "_v18_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v18] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "f1_mention_rate_distribution", "f2_cell_attrition", "f3_dissociation_scatter",
    ]
    expected_files = set()
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v18] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="Recognition x Recall Generalization \u2014 AIAS v0.18 Indie Fragrance",
    ...
    doc.build(story)
    print(f"[build_report_v18] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v18] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v18] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v18/reports/v18_indie_fragrance.pdf")
    args = ap.parse_args()
    build(
