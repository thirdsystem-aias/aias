...
# Make `import v1_8_consistency_instrument_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v1_8_consistency_instrument_content as content  # noqa: E402
...
# HYPOTHESIS_SCORING (v1.8 managerial): [{id, proposition, status}] → {heading, intro, rows}
...
# HYPOTHESIS_DETAILS: [{id, text}] (v1.8) or string → dict {heading, intro, items}
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "The Consistency Component (CPC) \u00b7 v1.8 \u00b7 June 2026")
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v1.8 is itself the governing methodology lock for this report
    REPORT_PROTOCOL_VERSION = "v1.8"
...
def build(*, debug_layout: bool = False,
    ...
    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "methodology" / "v1_8"
    # v1.8 figures live in reports/figs/v1_8/ (build_charts_v1_8.py default
    # fig-dir), the same fig_0*.pdf the academic paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v1_8")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v1_8_consistency_instrument_report.pdf")
    base_pdf = output_path.parent / "_v1_8_base.pdf"
