#!/usr/bin/env python3
"""
v0.34 Provider-Asymmetric CPC — brand-format report typesetting pipeline.

Third System managerial-register companion to the v0.34 SSRN paper. Renders the
P1-P5 propositions (P1/P2/P3 charted, P4/P5 text-only) from
v34_longitudinal_t1_t2_omnibus_content.py, with three report-register figures from
build_charts_v34.py (report mode):
...
Charts live in reports/figs/v34/report/; output to osf/v34/reports/.

Run:
    python3 ~/aias/scripts/build_charts_v34.py     # both registers
    python3 ~/aias/reports/build_report_v34.py
"""
...
# Make `import v34_longitudinal_t1_t2_omnibus_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v34_longitudinal_t1_t2_omnibus_content as content  # noqa: E402
...
# v0.34: the chart-bearing findings ARE the P-propositions (P1/P2/P3 charted,
...
# HYPOTHESIS_SCORING (v0.34): [{id, proposition, verdict, basis}] → {heading, intro, rows}
...
# HYPOTHESIS_DETAILS (v0.34 dict P1-P5) is consumed into the P-findings above;
...
if isinstance(content.CLOSING, dict):
    ...
    content.CLOSING.setdefault("methodology_log", "v0.34 (SSRN pending)")
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "AIAS\u2122 Consistency (CPC) \u00b7 v0.34 \u00b7 June 2026")
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    ...
    table = {
        "asym":  ("report_fig_01.pdf", "v34_asym"),
        "gate":  ("report_fig_02.pdf", "v34_gate"),
        "ranks": ("report_fig_03.pdf", "v34_ranks"),
        "phantom": ("report_fig_04.pdf", "v34_phantom"),
    }
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v34] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v34] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v34"
    # v0.34 figures live in reports/figs/v34/ (build_charts_v34.py default
    # fig-dir), the same chart_0*.pdf the SSRN paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v34" / "report")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v34_longitudinal_t1_t2_omnibus_report.pdf")
    base_pdf = output_path.parent / "_v34_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v34] chart pre-flight (looking in {chart_dir})")
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v34] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc.build(story)
    print(f"[build_report_v34] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v34] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v34] page count: {final_pages}")
