#!/usr/bin/env python3
"""
v0.27 Automotive — typesetting pipeline.
...
Brand-format report companion to the v0.27 SSRN paper (SSRN TBD).

Forked from build_report_v21.py with surgical changes:
  - Imports v27_automotive_content instead of v21_cosmetics_content
  - HERO_FIGURE_CAPTIONS rewritten as v0.27 templates (4 entries; the
    fourth — f4_phantom_defunct — is net new for v0.27)
  - _slot_lookup table updated for the 4 v0.27 brand-format chart filenames
    (all produced by build_charts_v27.py at reports/figs/v27/):
  ...
  - CHART_FIGSIZE_IN extended with v0.27 figsize keys (4 entries; v0.21
    entries preserved for cross-version reuse)
  - Header right text updated: "Phantom Brand Persistence · v0.27 · May 2026"
  - Citation updated: v0.27 automotive, SSRN TBD
  - Output filename: v27_b2b_saas_convergent_validity_report.pdf
  - REPORT_PROTOCOL_VERSION: v1.5 → v1.6 (first prospective v1.6 phase)
  - Chart directory: reports/figs/v27/
  - Deposit root: osf/v27/
  - Page count target band: 8-14 pages (v0.27 has 4 findings vs v0.21's 3;
...
Inputs:
  ...
  - reports/figs/v27/chart_01_*.pdf      (4 brand-format charts; all from
                                          build_charts_v27.py)

Output:
  - osf/v27/reports/v27_b2b_saas_convergent_validity_report.pdf

Run for v0.27:
    python3 ~/aias/scripts/build_charts_v27.py
    python3 ~/aias/reports/build_report_v27.py
...
# Make `import v27_b2b_saas_convergent_validity_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v27_b2b_saas_convergent_validity_content as content  # noqa: E402
import tsboilerplate as boilerplate  # noqa: E402

# ---------------------------------------------------------------------------
# Content adapter — reshape flat v0.27 content module into builder structures
# ---------------------------------------------------------------------------
...
if "tagline" not in content.COVER:
    content.COVER["tagline"] = "AIAS™ Measurement Program · Convergent Validity"
...
if isinstance(content.HYPOTHESIS_SCORING, list) and content.HYPOTHESIS_SCORING and isinstance(content.HYPOTHESIS_SCORING[0], dict):
    ...
    content.HYPOTHESIS_SCORING = {
        ...
        "intro": "Four pre-registered propositions for the v0.27 convergent-validity test of AIAS recall-SOM against the HubSpot AEO Grader (B2B SaaS, n = 24).",
        "rows": _rows,
    }
...
if isinstance(content.CLOSING, str):
    content.CLOSING = {
        ...
        "datasets": [
            "AIAS-side: osf.io/ec6wh/v27/data/v0.27_aias_side.csv",
            "Instrument I1: osf.io/ec6wh/v27/data/v0.27_I1_normalized.csv",
            "Scoring verdicts: osf.io/ec6wh/v27/v0.27_verdicts.json",
        ],
...
CHART_FIGSIZE_IN = {
    ...
    # v0.27 B2B SaaS brand-format report keys — 4 charts.
    "6_col_v27_recall_by_brand":  (7.50, 9.00),
    "6_col_v27_mlt_frequency":    (7.50, 5.50),
    "6_col_v27_identity_load":    (7.50, 5.50),
    "6_col_v27_mlc_by_model":     (7.50, 5.50),
    "6_col_v27_phantom_gap":      (7.50, 5.00),
    "hero":                       (7.50, 5.00),
...
class ChartReservation(Flowable):
    ...
    def wrap(self, available_w, available_h):
        ...
        if self.caption_p is not None:
            ...
            total_h = self.chart_h_pt + ch + 2   # was + 4 (v0.27 tightening)
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename: str, *,
        ...
        # v0.27 lead-page layout — REVERTED to v0.21 / aias_1_0 working pattern:
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Convergent Validity \u00b7 v0.27 \u00b7 May 2026")
...
HERO_FIGURE_CAPTIONS = {
    "f1_scatter_recall_sov": (
        "Figure 1 \u00b7 AIAS recall-SOM does not converge with the Grader\u2019s "
        "Share-of-Voice. Category-leadership recall (0\u2013100) vs. HubSpot "
        "Share-of-Voice (0\u201310) across 24 B2B SaaS brands; rho\u2009=\u20090.29 "
        "(n.s.). Much of the rank agreement is the live-versus-defunct split. "
        "Type-2 brands (specialist-salient, zero category recall) sit at the "
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.27 brand-format slot_key to (filename, figsize_key)."""
    table = {
        "f1_scatter_recall_sov": (
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.27 protocol version — v1.6 (Phantom Brand Persistence increment Inc3)
    ...
    citation_text = (
        ...
        "<i>A Pre-Registered Null on AI Share-of-Voice: Convergent Validity of "
        "AIAS Presence against Third-Party AI-Visibility Instruments (v0.27)</i>. "
        "Third System. thirdsystem.ai/v27"
    )
...
def overlay_charts(base_pdf_path: Path, manifest: ChartManifest,
    ...
    for slot in manifest.slots:
        ...
        # v0.27: scale = min(sx, sy) — was * 0.92 (8% safety margin). The
        ...
        # matched to build_charts_v27.py figsize, the margin is unnecessary.
        scale = min(sx, sy)
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v27] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v27] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v27"
    # Charts live in reports/figs/v27/ (matches build_charts_v27.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v27")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v27_b2b_saas_convergent_validity_report.pdf")
    base_pdf = output_path.parent / "_v27_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v27] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v27] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="What AI Presence Does Not Predict \u2014 AIAS v0.27",
    ...
    doc.build(story)
    print(f"[build_report_v27] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v27] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v27] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v27/reports/v27_b2b_saas_convergent_validity_report.pdf")
    args = ap.parse_args()
    build(
