#!/usr/bin/env python3
"""
v0.30 Automotive — typesetting pipeline.
...
Brand-format report companion to the v0.30 SSRN paper (SSRN TBD).

Forked from build_report_v21.py with surgical changes:
  - Imports v30_automotive_content instead of v21_cosmetics_content
  - HERO_FIGURE_CAPTIONS rewritten as v0.30 templates (4 entries; the
    fourth — f4_phantom_defunct — is net new for v0.30)
  - _slot_lookup table updated for the 4 v0.30 brand-format chart filenames
    (all produced by build_charts_v30.py at reports/figs/v30/):
  ...
  - CHART_FIGSIZE_IN extended with v0.30 figsize keys (4 entries; v0.21
    entries preserved for cross-version reuse)
  - Header right text updated: "Phantom Brand Persistence · v0.30 · May 2026"
  - Citation updated: v0.30 automotive, SSRN TBD
  - Output filename: v30_cpc_instrument_pilot_report.pdf
  - REPORT_PROTOCOL_VERSION: v1.5 → v1.6 (first prospective v1.6 phase)
  - Chart directory: reports/figs/v30/
  - Deposit root: osf/v30/
  - Page count target band: 8-14 pages (v0.30 has 4 findings vs v0.21's 3;
...
Inputs:
  ...
  - reports/figs/v30/chart_01_*.pdf      (4 brand-format charts; all from
                                          build_charts_v30.py)

Output:
  - osf/v30/reports/v30_cpc_instrument_pilot_report.pdf

Run for v0.30:
    python3 ~/aias/scripts/build_charts_v30.py
    python3 ~/aias/reports/build_report_v30.py
...
# Make `import v1_7_cpc_consistency_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v1_7_cpc_consistency_content as content  # noqa: E402
...
if "tagline" not in content.COVER:
    content.COVER["tagline"] = content.COVER.get("program_line", "AIAS™ Measurement Program")
if "subtitle" not in content.COVER:
    content.COVER["subtitle"] = content.COVER.get("kicker", "")
...
# HYPOTHESIS_SCORING (v1.7 managerial): [{id, proposition, status}] → {heading, intro, rows}
# rows are 4-tuples (id, proposition, status_text, status_class) for a 3-column table.
if isinstance(content.HYPOTHESIS_SCORING, list) and content.HYPOTHESIS_SCORING and isinstance(content.HYPOTHESIS_SCORING[0], dict):
    _STATUS_CLASS = {"Established": "confirmed", "Observed": "partial", "Open": "descriptive"}
    _rows = []
    for h in content.HYPOTHESIS_SCORING:
        _status_text = h.get("status", h.get("verdict", ""))
        _status_class = _STATUS_CLASS.get(_status_text, "partial")
        _rows.append((h["id"], h.get("proposition", h.get("label", "")), _status_text, _status_class))
    content.HYPOTHESIS_SCORING = {
        "heading": "What this means for practitioners",
        "intro": "Five propositions, drawn from the locked result and written for brand teams rather than reviewers.",
        "rows": _rows,
    }

# HYPOTHESIS_DETAILS: [{id, text}] (v1.7) or string → dict {heading, intro, items}
if isinstance(content.HYPOTHESIS_DETAILS, list) and content.HYPOTHESIS_DETAILS and isinstance(content.HYPOTHESIS_DETAILS[0], dict):
    content.HYPOTHESIS_DETAILS = {
        "heading": "The propositions in full",
        "intro": "",
        "items": [(d["id"], d["text"]) for d in content.HYPOTHESIS_DETAILS],
    }
elif isinstance(content.HYPOTHESIS_DETAILS, str):
...
if isinstance(content.CLOSING, str):
    content.CLOSING = {
        ...
        "datasets": [
            "Per-brand CPC: osf.io/ec6wh/methodology/v1_7/data/v1_7_cpc.csv",
            "Verdicts: osf.io/ec6wh/methodology/v1_7/v1_7_cpc_verdicts.json",
            "Scoring code: osf.io/ec6wh/methodology/v1_7/scoring/score_v1_7.py",
        ],
        "methodology_log": "v1.7 (SSRN 6878818)",
...
if isinstance(content.CLOSING, dict):
    ...
    content.CLOSING.setdefault("methodology_log", "v1.7 (SSRN 6878818)")
...
CHART_FIGSIZE_IN = {
    ...
    # v1.7 CPC figures — native dims from fig_0*.pdf mediaboxes
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "The Consistency Component (CPC) \u00b7 v1.7 \u00b7 June 2026")
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    citation_text = (
        ...
        "Coefficient-of-Variation as the Consistency Component of AIAS (v1.7)</i>. "
        "Third System (SSRN 6878818). thirdsystem.ai/methodology/v1_7"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v30] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v30] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "methodology" / "v1_7"
    # v1.7 figures live in reports/figs/v1_7/ (build_charts_v1_7.py default
    # fig-dir), the same fig_0*.pdf the academic paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v1_7")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v1_7_cpc_consistency_report.pdf")
    base_pdf = output_path.parent / "_v1_7_base.pdf"
    ...
    doc = V15DocTemplate(
        ...
        title="Measuring Presence Twice \u2014 AIAS v1.7 CPC",
    ...
    doc.build(story)
    print(f"[build_report_v30] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v30] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v30] page count: {final_pages}")
