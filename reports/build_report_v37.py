#!/usr/bin/env python3
"""
v0.37 Provider-Asymmetric CPC — brand-format report typesetting pipeline.

Third System managerial-register companion to the v0.37 SSRN paper. Renders the
P1-P5 propositions (P1/P2/P3 charted, P4/P5 text-only) from
v37_cpc_identity_load_moderator_content.py, with three report-register figures from
build_charts_v37.py (report mode):
...
Charts live in reports/figs/v37/report/; output to osf/v37/reports/.

Run:
    python3 ~/aias/scripts/build_charts_v37.py     # both registers
    python3 ~/aias/reports/build_report_v37.py
"""
...
# Make `import v37_cpc_identity_load_moderator_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v37_cpc_identity_load_moderator_content as content  # noqa: E402
...
if "tagline" not in content.COVER:
    content.COVER["tagline"] = content.COVER.get("eyebrow", "AIAS™ Measurement Program")
content.COVER.setdefault("subtitle", "")
content.COVER.setdefault("deck", "")
if "date" not in content.COVER:
    content.COVER["date"] = "June 2026"
...
# v0.37: the chart-bearing findings ARE the P-propositions (P1/P2/P3 charted,
...
# HYPOTHESIS_SCORING (v0.37): [{id, proposition, verdict, basis}] → {heading, intro, rows}
if isinstance(content.HYPOTHESIS_SCORING, list) and content.HYPOTHESIS_SCORING and isinstance(content.HYPOTHESIS_SCORING[0], dict):
    # honesty constraint: positive -> confirmed slot, neutral -> descriptive slot;
    # nothing uses the disconfirmed (red) slot (none was cleanly falsified).
    _SC_MAP = {"positive": "confirmed", "neutral": "descriptive"}
    _rows = []
    for h in content.HYPOTHESIS_SCORING:
        _status = f"{h.get('verdict', '')} — {h.get('qualifier', '')}".strip(" —")
...
# HYPOTHESIS_DETAILS (v0.37 dict P1-P5) is consumed into the P-findings above;
# render nothing separately (avoids duplicating each proposition's detail).
content.HYPOTHESIS_DETAILS = {"heading": "", "intro": "", "items": []}
...
if isinstance(content.CLOSING, str):
    content.CLOSING = {
        ...
        "datasets": [
            "Verdicts: osf.io/ec6wh/v37/v37_verdicts.json",
            "Two-wave acquisition data: osf.io/ec6wh/v37/data/",
            "Scoring code: osf.io/ec6wh/v37/scripts/score_v37.py",
            "Pre-registration: osf.io/ec6wh/v37/prereg/ (tag v0.37-prereg-r1)",
        ],
        "methodology_log": "v0.37 (SSRN pending)",
        "closing_text": content.CLOSING,
    }

# CLOSING shipped as a dict by the content module: normalize byline_long /
# datasets to lists. build_closing_story iterates these; a bare string would be
# iterated CHARACTER-BY-CHARACTER (one Paragraph per char → many near-blank
# pages). This guards against that regardless of how the module authored them.
if isinstance(content.CLOSING, dict):
    for _k in ("byline_long", "datasets"):
        _v = content.CLOSING.get(_k)
    ...
    content.CLOSING.setdefault("methodology_log", "v0.37 (SSRN pending)")
...
CHART_FIGSIZE_IN = {
    # v0.37 phantom-CPC report charts (native mediabox dims; aspect re-derived by ChartReservation)
    "v37_gate_flip":  (7.662, 5.100),
    "v37_saturation": (8.769, 4.700),
    "v37_raw":        (7.275, 5.100),
    "v37_cross":      (9.329, 5.100),
    "v37_t2":         (7.275, 5.100),
    "v37_roster":     (8.225, 5.100),
    ...
    # v0.37 longitudinal t1->t2 report figures — native dims from report_fig_0*.pdf mediaboxes
    ...
    "v37_asym":                   (8.674, 3.300),   # report_fig_01 CPC t1/t2 stability (624.5x237.6pt)
    "v37_gate":                   (7.675, 5.100),   # report_fig_02 residual gate saturation (552.6x367.2pt)
    "v37_ranks":                  (8.561, 3.300),   # report_fig_03 presence stability (616.4x237.6pt)
    "v37_phantom":                (7.200, 5.100),   # report_fig_04 phantom persistence (518.4x367.2pt)
}
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "AIAS\u2122 Consistency (CPC) \u00b7 v0.37 \u00b7 June 2026")
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    ...
    table = {
        "roster":     ("chart_06_roster_attrition.pdf",          "v37_roster"),
        "gate_flip":  ("chart_01_gate_flip.pdf",                 "v37_gate_flip"),
        "saturation": ("chart_02_recognition_saturation.pdf",    "v37_saturation"),
        "raw":        ("chart_03_raw_manipulation_check.pdf",     "v37_raw"),
        "cross":      ("chart_04_cross_substrate_concordance.pdf","v37_cross"),
        "t2":         ("chart_05_t2_stability.pdf",               "v37_t2"),
    }
...
def build_hypothesis_scoring_story(styles: dict) -> list:
    ...
    for h_id, proposition, status_text, status_class in content.HYPOTHESIS_SCORING["rows"]:
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v37] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v37] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v37"
    # v0.37 figures live in reports/figs/v37/ (build_charts_v37.py default
    # fig-dir), the same chart_0*.pdf the SSRN paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v37")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v37_cpc_identity_load_moderator_report.pdf")
    base_pdf = output_path.parent / "_v37_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v37] chart pre-flight (looking in {chart_dir})")
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v37] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="The Question the Instrument Couldn\u2019t Answer \u2014 AIAS Naive-Phantom x CPC (v0.37)",
    ...
    doc.build(story)
    print(f"[build_report_v37] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v37] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v37] page count: {final_pages}")
