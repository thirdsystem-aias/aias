#!/usr/bin/env python3
"""
Phase 2 BBB — Phantom Brand Persistence v0.7 typesetting pipeline.

Adapted from build_report.py (v0.6 cross-category) with minimal changes:
  - Imports v07_bbb_content instead of v06_content
  - Section label "PATTERN" → "FINDING"
  - Hero check matches v07 hero slot keys
  - HERO_FIGURE_CAPTIONS rewritten for v07 figures
  - _slot_lookup table updated for chart_v07_*.pdf filenames
  - Header right text, citation, output filename, doc metadata updated
  - THREE_MODES rendering skipped (v07 content has no THREE_MODES)
  - Aggregate matrix replaced by hypothesis-scoring table page
...
Inputs:
  ...
  - reports/output/chart_v07_*.pdf       (5 charts from build_charts_v07_bbb.py)

Output:
  - reports/output/v07_phantom_persistence.pdf

The v0.6 build_report.py is left untouched. Run this for Phase 2 BBB:

    python3 build_charts_v07_bbb.py
    python3 build_report_v07.py

Locked rules followed (same as v0.6):
...
import v07_bbb_content as content  # noqa: E402
...
class V06DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Phantom Brand Persistence \u00b7 v0.7 \u00b7 4 May 2026")
...
def build_leaderboards_spread(styles: dict, manifest: ChartManifest,
    ...
    s.append(KeepTogether([
        Paragraph("Household goods leaderboard", styles["h1"]),
        Paragraph(
            "Twenty-one retailers ordered by AI Presence. Bed Bath &amp; Beyond and "
            "Pier 1 are highlighted as phantom test subject and structural "
            "comparator \u2014 the foundation for the five findings that follow.",
            styles["body_lead"]),
    ]))
    s.append(Spacer(1, 8))
    s.append(ChartReservation(
        ...
        debug=debug,
        caption=("Figure A \u00b7 Twenty-one household-goods retailers by AI "
                 "Presence. BBB at #11 (36.8%) sits between Williams-Sonoma "
                 "and West Elm; Pier 1 at #21 (0.0%) appears in zero of 288 "
                 "measurements. The phantom signal is structurally specific."),
...
HERO_FIGURE_CAPTIONS = {
    "hero_f1_comparator": (
        "Figure 1 \u00b7 BBB and Pier 1 share a fate (collapse \u2192 online-only "
        "revival) but differ on years-since-collapse and pre-collapse footprint. "
        "BBB at 38.2% phantom, Pier 1 at exactly 0.0% across all 288 measurements."
    ),
    "hero_f2_valence": (
        "Figure 2 \u00b7 Per-model valence breakdown of BBB mentions. Naive "
        "phantom is 1.7% of all measurements; caveated phantom is 25.0%. AI knows "
        "the entity has changed in 88% of mentions \u2014 it surfaces the brand "
        "anyway, with disclosure."
    ),
    "hero_f4_temporal": (
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v07] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v07] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    # Preconditions
    chart_dir = chart_dir or OUTPUT_DIR
    output_path = output_path or (OUTPUT_DIR / "v07_phantom_persistence.pdf")
    base_pdf = OUTPUT_DIR / "_v07_base.pdf"
    ...
    if chart_dir.exists():
        ...
        if actual_charts:
            ...
            if unexpected:
                print(f"[build_report_v07] chart files in dir not matched by any slot:")
                for n in unexpected:
                    print(f"  [UNUSED ] {n}")
    ...
    doc = V06DocTemplate(
        ...
        title="Phantom Brand Persistence — AI Presence Index v0.7",
    ...
    # data summary). Uses HYPOTHESIS_SCORING from v07_bbb_content. Renders on
    # the SPREAD template because the scoring table needs the full 540pt
    ...
    # Build base PDF
    doc.build(story)
    print(f"[build_report_v07] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v07] FINAL PDF written: {output_path}")

    # Page count check
    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v07] page count: {final_pages}")
