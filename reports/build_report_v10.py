...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v10] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v10] typography: {font.name} "
    ...
    print(f"[build_report_v10] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "hero_f1_valence", "hero_f2_h2_split", "hero_f3_decoupling",
    ]
    expected_files = set()
    ...
    if chart_dir.exists():
        ...
        if actual_charts:
            ...
            if unexpected:
                print(f"[build_report_v10] chart files in dir not matched by any v10 slot:")
                for n in unexpected:
                    print(f"  [UNUSED ] {n}")
    ...
    # Build base PDF
    doc.build(story)
    print(f"[build_report_v10] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v10] FINAL PDF written: {output_path}")
    ...
    print(f"[build_report_v10] page count: {final_pages}")
