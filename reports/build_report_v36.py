#!/usr/bin/env python3
"""
v0.35 Provider-Asymmetric CPC — brand-format report typesetting pipeline.

Third System managerial-register companion to the v0.35 SSRN paper. Renders the
P1-P5 propositions (P1/P2/P3 charted, P4/P5 text-only) from
v35_phantom_cpc_omnibus_content.py, with three report-register figures from
build_charts_v35.py (report mode):

  asym  -> report_fig_01.pdf   (P1 eta^2 permutation null)
  gate  -> report_fig_02.pdf   (P2 recall vs recognition arms; saturation collapse)
  ranks -> report_fig_03.pdf   (P3 within-pair consistency rank heatmap)

Charts live in reports/figs/v35/report/; output to osf/v35/reports/.

Run:
    python3 ~/aias/scripts/build_charts_v35.py     # both registers
    python3 ~/aias/reports/build_report_v35.py
"""
...
# Make `import v35_phantom_cpc_omnibus_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v36_cpc_regime_emergence_content as content  # noqa: E402
import tsboilerplate as boilerplate  # noqa: E402

# ---------------------------------------------------------------------------
# Content adapter — reshape flat v0.30 content module into builder structures
# ---------------------------------------------------------------------------

# COVER: add keys the cover story builder expects
content.COVER.setdefault("byline_short", "Pablo Ulpiano González Castro")
content.COVER.setdefault("tagline", content.COVER.get("eyebrow", "AIAS™ Measurement Program"))
content.COVER.setdefault("subtitle", "")
content.COVER.setdefault("deck", "")
content.COVER.setdefault("date", "June 2026")
...
# LEAD_DECK: authored as a string (rendered as one deck block with <br/> lines).
...
# PATTERNS: v0.36 authors the five findings DIRECTLY (P1->chart_02, P2 text-only,
# P3->chart_03, P4->chart_01, P5->chart_04). Normalize keys; no rebuild from scoring.
_PATTERN_PROSE = ""   # no separate in-summary prose for v0.36
content.PATTERNS = [{
    "number": p["number"],
    "title": p["title"],
    "paragraphs": p.get("paragraphs", []),
    "chart_slot": p.get("chart_slot"),
    "chart_after_text": p.get("chart_after_text", False),
    "no_finding_label": p.get("no_finding_label", False),
} for p in content.PATTERNS]

# HYPOTHESIS_SCORING (v0.36) authored as {heading, intro, columns, rows, net};
# HYPOTHESIS_DETAILS authored as {heading, intro, items}. Both consumed directly
# by build_hypothesis_scoring_story (4-column table + per-hypothesis prose).
...
# CLOSING: string → dict {byline_long, datasets, methodology_log, closing_text}
if isinstance(content.CLOSING, str):
    content.CLOSING = {
        ...
        "datasets": [
            ...
            "Scoring code: osf.io/ec6wh/v36/ (scripts/score_v0_36.py)",
            "Pre-registration: osf.io/ec6wh/v36/prereg/ (tag v0.36-prereg-r2)",
        ],
        "methodology_log": "v0.36 (re-analysis; CPC characterization quantity, v1.7)",
...
CHART_FIGSIZE_IN = {
    ...
    # v0.30 B2B SaaS brand-format report keys — 4 charts.
    "6_col_v30_recall_by_brand":  (7.50, 9.00),
    "6_col_v30_mlt_frequency":    (7.50, 5.50),
    "6_col_v30_identity_load":    (7.50, 5.50),
    "6_col_v30_mlc_by_model":     (7.50, 5.50),
    "6_col_v30_phantom_gap":      (7.50, 5.00),
    "hero":                       (7.50, 5.00),
    "hero_short":                 (7.50, 3.50),
    # v0.35 longitudinal t1->t2 report figures — native dims from report_fig_0*.pdf mediaboxes
    ...
    "v35_asym":                   (8.674, 3.300),   # report_fig_01 CPC t1/t2 stability (624.5x237.6pt)
    "v35_gate":                   (7.675, 5.100),   # report_fig_02 residual gate saturation (552.6x367.2pt)
    "v35_ranks":                  (8.561, 3.300),   # report_fig_03 presence stability (616.4x237.6pt)
    "v35_phantom":                (7.200, 5.100),   # report_fig_04 phantom persistence (518.4x367.2pt)
    # v0.36 CPC regime-emergence report charts (native mediabox widths; aspect
    # re-derived by ChartReservation from each chart PDF mediabox).
    "v36_omnibus":      (7.431, 4.775),   # chart_01_omnibus_structure
    "v36_inheritance":  (7.000, 4.595),   # chart_02_inheritance_floor (slightly capped so P1+P2 share a page)
    "v36_saturation":   (7.431, 4.875),   # chart_03_saturation_reversal
    "v36_residual":     (7.498, 4.775),   # chart_04_residual_divergence
    "v36_matrix":       (7.275, 4.875),   # chart_05_verdict_matrix
}
...
class ChartReservation(Flowable):
    def __init__(self, slot_key: str, chart_path: Path,
        ...
        # v0.30 CV.04: the dissociation chart is intentionally tall (24 brands,
        # one row each — native ~9.1in). Cap at 430pt so the FINDING heading +
        # 2-col body text + chart + caption all fit one 720pt 'spread' frame
        # (heading ~50 + text ~90 + chart 430 + caption ~60 = 630 < 720); a
        # taller cap (520) pushed the body text to an ~85%-blank orphan page.
        # The chart letterboxes (uniform min-scale) at any cap below its native
        # height, so 430 vs 520 is a slightly smaller render, never distortion.
        nominal_h_pt = 430.0 if raw_h_pt > 430.0 else raw_h_pt

        # v0.30: auto-size reservation height to match the actual chart PDF's
        # aspect ratio. bbox_inches='tight' in build_charts_v30 produces
    ...
    def wrap(self, available_w, available_h):
        ...
        if self.caption_p is not None:
            ...
            total_h = self.chart_h_pt + ch + 2   # was + 4 (v0.30 tightening)
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename: str, *,
        ...
        # v0.30 lead-page layout — REVERTED to v0.21 / aias_1_0 working pattern:
        ...
        LEAD_TOP_H = 205.0   # v0.36: shrunk from 340 so the 4-para EXEC_SUMMARY
                             # fits on the lead page (no near-empty overflow page)
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "AIAS\u2122 CPC Regime Emergence \u00b7 v0.36 \u00b7 June 2026")
...
def build_cover_story(styles: dict[str, ParagraphStyle]) -> list:
    ...
    if content.COVER.get("deck"):
        s.append(Spacer(1, 12))
        s.append(Paragraph(content.COVER["deck"], styles["cover_byline"]))
...
def build_what_we_measured_story(styles, manifest=None, chart_dir=None, debug=False) -> list:
    s = []
    # chart_05 (verdict matrix) renders with the executive narrative — right after
    # EXEC_SUMMARY, leading the methodology spread.
    if manifest is not None and chart_dir is not None:
        _fn, _fk = _slot_lookup("matrix")
        _cp = (chart_dir / _fn) if _fn else None
        if _cp is not None and _cp.exists():
            _w, _h = CHART_FIGSIZE_IN[_fk]
            _cw = CONTENT_W / 72.0
            if _w > _cw:
                _h *= _cw / _w; _w = _cw
            s.append(ChartReservation("matrix", _cp, _w, _h, manifest, debug=debug,
                     caption=HERO_FIGURE_CAPTIONS.get("matrix", ""),
...
HERO_FIGURE_CAPTIONS = {
    "matrix": (
        "The locked verdict matrix. The phase pre-registered the top-left cell - "
        "borrowed structure - and landed in the opposite corner: unstructured. "
        "Inheritance and saturation both miss; the residual-independence call was the "
        "one held prediction."
    ),
    "inheritance": (
        "Where the test could run, it ran on nine brands - the computability floor "
        "erased the other fifteen - and a single forced cluster left zero agreement "
        "with the presence regime. The instrument, not the brands, set the limit."
    ),
    "saturation_rev": (
        "Saturated categories carry MORE cross-model dispersion, not less; the dashed "
        "means and the arrow mark the pre-registered direction reversing. Read as "
        "post-hoc mechanism - a ratio metric whose denominator shrinks in low-recall "
        "tails - not as a finding."
    ),
    "omnibus": (
        "Pooled across 112 brand-units, three clusters rise above the permutation null - "
        "but the silhouette sits a hair under the locked 0.25 bar. Real enough that the "
        "next instrument must explain it; not strong enough to score."
    ),
    "residual": (
        "Two gap curves. The locked primary (left) finds no residual structure; the "
        "sensitivity-only arm that removes recall volume entirely (right) does. The one "
        "place structure survives is the one place the mean is gone."
    ),
}
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    ...
    table = {
        "omnibus":        ("chart_01_omnibus_structure.pdf",    "v36_omnibus"),
        "inheritance":    ("chart_02_inheritance_floor.pdf",    "v36_inheritance"),
        "saturation_rev": ("chart_03_saturation_reversal.pdf",  "v36_saturation"),
        "residual":       ("chart_04_residual_divergence.pdf",  "v36_residual"),
        "matrix":         ("chart_05_verdict_matrix.pdf",       "v36_matrix"),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    s: list = []
    # Charted findings start their own page; a chartless finding (P2) flows onto
    # the previous finding's page instead of opening a near-empty page of its own.
    if pattern.get("chart_slot"):
        s.append(PageBreak())
    else:
        s.append(Spacer(1, 8))
    ...
    if not pattern.get("no_finding_label"):
        _head.append(Paragraph(f"PROPOSITION P{pattern['number']}", styles["pattern_number"]))
    ...
    if chart_path is not None and figsize_key is not None:
        ...
        # Clamp reservation width to the page content column. The CV.04 charts are
        # saved with bbox_inches='tight', so several exceed the 7.5in column
        # (verdict_bands 8.6in, dissociation 7.9in). Without this clamp the chart
        # overflows the right margin and off-axis labels (UNDETERMINED/CONFIRMED)
        # are clipped. Height scales proportionally; ChartReservation re-derives
        # the true footprint from the PDF mediabox, so aspect is preserved.
        _content_w_in = CONTENT_W / 72.0
        if w_in > _content_w_in:
            h_in = h_in * (_content_w_in / w_in)
            w_in = _content_w_in
        # v0.30 CV.04 hero slot names (all four findings carry a hero chart)
        is_hero = slot in ("matrix", "omnibus", "inheritance", "saturation_rev", "residual")
...
def build_hypothesis_scoring_story(styles: dict) -> list:
    ...
    warm = HexColor("#BD4718")
    gray = HexColor("#888888")
    lavender = HexColor("#BBB9DD")

    header_style = ParagraphStyle(
        "scoring_header", parent=styles["body"],
        fontName=styles["body_lead"].fontName,
        fontSize=8.5, textColor=indigo, leading=11,
    )
    cell_style = ParagraphStyle(
        "scoring_cell", parent=styles["body"], fontSize=7.8, leading=10,
    )
    h_cell_style = ParagraphStyle(
        "scoring_h", parent=cell_style, fontName=header_style.fontName,
    )
    pred_style = ParagraphStyle("scoring_pred", parent=cell_style, textColor=gray)

    def verdict_style(cls):
        if cls == "net":
            return ParagraphStyle("v_net", parent=cell_style, textColor=indigo,
                                  fontName=h_cell_style.fontName)
        if cls == "match":
            return ParagraphStyle("v_match", parent=cell_style, textColor=indigo)
        return ParagraphStyle("v_miss", parent=cell_style, textColor=warm)

    cols = content.HYPOTHESIS_SCORING.get(
        "columns", ["Hypothesis", "Scope", "Predicted", "Verdict"])
    table_data = [[Paragraph(f"<b>{c}</b>", header_style) for c in cols]]

    all_rows = list(content.HYPOTHESIS_SCORING["rows"]) + [content.HYPOTHESIS_SCORING["net"]]
    for hyp, scope, predicted, verdict, cls in all_rows:
        is_net = cls == "net"
        verdict_markup = f"<b>{verdict}</b>" if is_net else verdict
        table_data.append([
            Paragraph(f"<b>{hyp}</b>", h_cell_style),
            Paragraph(scope, cell_style),
            Paragraph(f"<i>{predicted}</i>", pred_style),
            Paragraph(verdict_markup, verdict_style(cls)),
        ])

    tbl = Table(
        table_data,
        colWidths=[112, 126, 112, 190],   # sums to CONTENT_W (540pt)
        style=TableStyle([
            ("LINEABOVE",  (0, 0), (-1, 0), 1.2, indigo),
            ("LINEBELOW",  (0, 0), (-1, 0), 0.5, indigo),
            ("BOTTOMPADDING", (0, 0), (-1, 0), 5),
            ("TOPPADDING",    (0, 0), (-1, 0), 5),
            ("LINEBELOW",  (0, 1), (-1, -3), 0.3, lavender),
            ("LINEABOVE",  (0, -1), (-1, -1), 0.8, indigo),   # Net-row separator
    ...
    if hasattr(content, "HYPOTHESIS_DETAILS") and content.HYPOTHESIS_DETAILS.get("items"):
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v1.7 is itself the governing methodology lock for this report
    REPORT_PROTOCOL_VERSION = "v1.7"
...
def overlay_charts(base_pdf_path: Path, manifest: ChartManifest,
    ...
    for slot in manifest.slots:
        ...
        # v0.30: scale = min(sx, sy) — was * 0.92 (8% safety margin). The
        ...
        # matched to build_charts_v30.py figsize, the margin is unnecessary.
        scale = min(sx, sy)
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v35] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v35] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V36_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v36"
    # v0.36 figures live in reports/figs/v36/ (build_charts_v36.py default
    # fig-dir), the same chart_0*.pdf the SSRN paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v36")
    output_path = output_path or (V36_DEPOSIT_ROOT / "reports" / "v36_cpc_regime_emergence_report.pdf")
    base_pdf = output_path.parent / "_v36_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v36] chart pre-flight (looking in {chart_dir})")
    expected_slots = ["matrix", "omnibus", "inheritance", "saturation_rev", "residual"]
    expected_files = set()
    ...
    doc = V15DocTemplate(
        ...
        title="The Structure That Wasn\u2019t There \u2014 AIAS CPC Regime Emergence (v0.36)",
    ...
    if _PATTERN_PROSE:
        ...
        story.append(Paragraph("In summary", styles["h1"]))
    ...
    doc.build(story)
    print(f"[build_report_v35] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v35] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v35] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v30/reports/v30_cpc_instrument_pilot_report.pdf")
    args = ap.parse_args()
    build(
