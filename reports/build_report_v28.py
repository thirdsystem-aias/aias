#!/usr/bin/env python3
"""
v0.28 Automotive — typesetting pipeline.
...
Brand-format report companion to the v0.28 SSRN paper (SSRN TBD).

Forked from build_report_v21.py with surgical changes:
  - Imports v28_automotive_content instead of v21_cosmetics_content
  - HERO_FIGURE_CAPTIONS rewritten as v0.28 templates (4 entries; the
    fourth — f4_phantom_defunct — is net new for v0.28)
  - _slot_lookup table updated for the 4 v0.28 brand-format chart filenames
    (all produced by build_charts_v28.py at reports/figs/v28/):
  ...
  - CHART_FIGSIZE_IN extended with v0.28 figsize keys (4 entries; v0.21
    entries preserved for cross-version reuse)
  - Header right text updated: "Phantom Brand Persistence · v0.28 · May 2026"
  - Citation updated: v0.28 automotive, SSRN TBD
  - Output filename: v28_tech_brand_discriminant_report.pdf
  - REPORT_PROTOCOL_VERSION: v1.5 → v1.6 (first prospective v1.6 phase)
  - Chart directory: reports/figs/v28/
  - Deposit root: osf/v28/
  - Page count target band: 8-14 pages (v0.28 has 4 findings vs v0.21's 3;
...
Inputs:
  ...
  - reports/figs/v28/chart_01_*.pdf      (4 brand-format charts; all from
                                          build_charts_v28.py)

Output:
  - osf/v28/reports/v28_tech_brand_discriminant_report.pdf

Run for v0.28:
    python3 ~/aias/scripts/build_charts_v28.py
    python3 ~/aias/reports/build_report_v28.py
...
# Make `import v28_tech_brand_discriminant_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v28_tech_brand_discriminant_content as content  # noqa: E402
import tsboilerplate as boilerplate  # noqa: E402

# ---------------------------------------------------------------------------
# Content adapter — reshape flat v0.28 content module into builder structures
# ---------------------------------------------------------------------------
...
if isinstance(content.HYPOTHESIS_SCORING, list) and content.HYPOTHESIS_SCORING and isinstance(content.HYPOTHESIS_SCORING[0], dict):
    ...
    content.HYPOTHESIS_SCORING = {
        ...
        "intro": "Three pre-registered hypotheses, scored once against the locked verdicts on the frozen 24-brand Technology panel (seed 280400; blinding released only at scoring).",
        "rows": _rows,
    }
...
if isinstance(content.CLOSING, str):
    content.CLOSING = {
        ...
        "datasets": [
            "Human norms: osf.io/ec6wh/v28/prereg/v0_28_brand_validator.csv",
            "AIAS Presence: osf.io/ec6wh/v28/data/v0.28_presence.csv",
            "Scoring verdicts: osf.io/ec6wh/v28/v28_verdicts.json",
        ],
...
if isinstance(content.CLOSING, dict):
    ...
    content.CLOSING.setdefault("methodology_log", "v1.6 (SSRN 6816340)")
...
CHART_FIGSIZE_IN = {
    ...
    # v0.28 B2B SaaS brand-format report keys — 4 charts.
    "6_col_v28_recall_by_brand":  (7.50, 9.00),
    "6_col_v28_mlt_frequency":    (7.50, 5.50),
    "6_col_v28_identity_load":    (7.50, 5.50),
    "6_col_v28_mlc_by_model":     (7.50, 5.50),
    "6_col_v28_phantom_gap":      (7.50, 5.00),
    "hero":                       (7.50, 5.00),
    "hero_short":                 (7.50, 3.50),
    # v0.28 CV.04 charts — native dims from chart_28_*.pdf mediaboxes
    ...
    "v28_verdict_bands":          (8.61, 3.50),
    "v28_scatters":               (6.96, 5.10),
    "v28_dissociation":           (7.88, 9.10),
    "v28_composition":            (6.75, 5.10),
}
...
class ChartReservation(Flowable):
    def __init__(self, slot_key: str, chart_path: Path,
        ...
        # v0.28 CV.04: the dissociation chart is intentionally tall (24 brands,
        ...
        # v0.28: auto-size reservation height to match the actual chart PDF's
        # aspect ratio. bbox_inches='tight' in build_charts_v28 produces
    ...
    def wrap(self, available_w, available_h):
        ...
        if self.caption_p is not None:
            ...
            total_h = self.chart_h_pt + ch + 2   # was + 4 (v0.28 tightening)
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename: str, *,
        ...
        # v0.28 lead-page layout — REVERTED to v0.21 / aias_1_0 working pattern:
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Discriminant Validity \u00b7 v0.28 \u00b7 May 2026")
...
def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.28 CV.04 brand-format slot_key to (filename, figsize_key).
    Charts are produced by build_charts_v28.py at reports/figs/ (figs root)."""
    table = {
        "f1_verdict_bands": (
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    if chart_path is not None and figsize_key is not None:
        ...
        if w_in > _content_w_in:
            ...
            w_in = _content_w_in
        # v0.28 CV.04 hero slot names (all four findings carry a hero chart)
        is_hero = slot in (
            "f1_verdict_bands",
            "f2_discriminant_scatters",
            "f3_dissociation",
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.28 protocol version — v1.6 (Phantom Brand Persistence increment Inc3)
    ...
    citation_text = (
        ...
        "Presence Against Human Brand Norms (v0.28 / CV.04)</i>. "
        "Third System. thirdsystem.ai/v28"
    )
...
def overlay_charts(base_pdf_path: Path, manifest: ChartManifest,
    ...
    for slot in manifest.slots:
        ...
        # v0.28: scale = min(sx, sy) — was * 0.92 (8% safety margin). The
        ...
        # matched to build_charts_v28.py figsize, the margin is unnecessary.
        scale = min(sx, sy)
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v28] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v28] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V22_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v28"
    # Charts live in reports/figs/ (figs root) — build_charts_v28.py default
    # fig-dir, and the same chart_28_*.pdf the academic paper references.
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs")
    output_path = output_path or (V22_DEPOSIT_ROOT / "reports" / "v28_tech_brand_discriminant_report.pdf")
    base_pdf = output_path.parent / "_v28_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v28] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "f1_verdict_bands",
        "f2_discriminant_scatters",
        "f3_dissociation",
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v28] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="What AI Presence Does Not Predict \u2014 AIAS v0.28",
    ...
    doc.build(story)
    print(f"[build_report_v28] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v28] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v28] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v28/reports/v28_tech_brand_discriminant_report.pdf")
    args = ap.parse_args()
    build(
