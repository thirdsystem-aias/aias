#!/usr/bin/env python3
"""
v0.19 Audiophile Headphones — typesetting pipeline.

C3 rescue phase (n=16, two same-IL cells split by audiophile headphone
product-line emergence year). Recognition × Recall dissociation construct
extends to a third substrate family with 3 cases; H_C3 UNDETERMINED at C2
boundary; cultural-channel asymmetry surfaces 3 Type 1 cases and 0 Type 2.
Brand-format report companion to the v0.19 SSRN paper (forthcoming).

Forked from build_report_v18.py with surgical changes:
  - Imports v19_audiophile_content instead of v18_indie_fragrance_content
  - Section label remains "FINDING"
  - HERO_FIGURE_CAPTIONS replaced with v0.19 brand-format figure captions (3)
  - _slot_lookup table updated for the 3 v0.19 brand-format chart filenames
    (all produced by build_charts_v19.py at reports/figs/v19/):
  ...
  - CHART_FIGSIZE_IN extended with v0.19 figsize keys (3 entries at 7.5x5.5,
    matching the native figsize used in build_charts_v19.py)
  - Header right text updated: "C3 Rescue · v0.19 · May 2026"
  - Citation updated: v0.19 audiophile headphones, SSRN forthcoming
  - Output filename: v19_audiophile.pdf
  - REPORT_PROTOCOL_VERSION stays v1.4 (no methodology change in v0.19)
  - Chart directory: reports/figs/v19/
  - Page count target band: 8-14 pages (v0.19 has 3 findings, same as v0.18)

Inputs:
  - brand/third_system_brand.json        (v1.5)
  - brand/design_tokens_template.json    (IDML extract)
  - reports/figs/v19/chart_01_*.pdf      (3 brand-format charts; all from
                                          build_charts_v19.py)

Output:
  - osf/v19/reports/v19_audiophile.pdf

Run for v0.19:
    python3 ~/aias/scripts/build_charts_v19.py
    python3 ~/aias/reports/build_report_v19.py

Locked rules followed (same as v0.6 - v0.18):
...
# Make `import v19_audiophile_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v19_audiophile_content as content  # noqa: E402
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.setFillColor(HexColor(self.palette.soft_black))
        # v0.19 header right text — updated from v0.18
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "C3 Rescue \u00b7 v0.19 \u00b7 May 2026")
...
# v0.19 brand-format report figure captions for the three v0.19 charts.
HERO_FIGURE_CAPTIONS = {
    "f1_cp_distribution": (
        ...
        "visibility. Cell A_Heritage spans C_P = 2 (Koss) to 6 (Sennheiser, "
        "Beyerdynamic, Grado, Audio-Technica) with four of eight brands at "
        "the ceiling \u2014 C_P modal share = 0.500, exactly at the C2 "
        "strict-less-than boundary. Cell B_Boutique has seven of eight "
        "brands at C_P = 6 (only Spirit Torino at 5/6) \u2014 modal share "
        "= 0.875, well above the C2 threshold. Both cells fail C2; H_C3 "
        "routes to UNDETERMINED per pre-registered DEVIATIONS Rule 4. "
        "Cell A's failure-by-one-brand exposes a C2 operationalization "
        "edge case for AIAS\u2122 1.0 methodology refinement."
    ),
    "f2_dissociation_scatter": (
        "Figure 2 \u00b7 Recognition \u00d7 Recall dissociation scatter \u2014 "
        "cumulative anchor base across three substrate families. Each "
        "colored point is one v0.19 audiophile-headphone brand: Phase A "
        "C_P (x-axis, 0\u20136) versus Phase B category-anchored mention "
        "count (y-axis, 0\u201318). Gray ghost markers show v0.18 indie-"
        "fragrance Iwachu cases (n = 9); black \u00d7 marks the v0.17 "
        "Japanese-cell Iwachu anchor. Shaded quadrant: Iwachu-pattern "
        "(C_P \u2265 5 \u2227 mentions \u2264 2). v0.19 contributes 3 new "
        "cases (ZMF Headphones, Spirit Torino, Final Audio \u2014 all "
        "Cell B_Boutique). The AIAS\u2122 v1.4 multi-component construct "
        "now spans three substrate families with 13 cumulative cases."
    ),
    "f3_channel_asymmetry": (
        "Figure 3 \u00b7 Channel asymmetry scatter. Per-brand category-"
        "anchored mentions (x-axis, q1\u2013q3 \u00d7 6 models = max 18) "
        "versus cultural-footprint mentions (y-axis, q4\u2013q6 \u00d7 6 "
        "models = max 18). Lower-right shaded quadrant: Type 1 (category-"
        "channel-preferred; category-anchored \u2265 5 \u2227 cultural-"
        "footprint \u2264 2). Upper-left quadrant: Type 2 (cultural-"
        "channel-preferred; category-anchored \u2264 2 \u2227 cultural-"
        "footprint \u2265 5). Three Type 1 cases in v0.19: Audeze (13/18, "
        "2/18), HiFiMan (12/18, 1/18), Dan Clark Audio (5/18, 0/18) \u2014 "
        "all Cell B_Boutique. Zero Type 2 cases. The Type 2 quadrant "
        "emptiness reflects panel composition (no mass-consumer brands "
        "in the Heritage \u00d7 Boutique audiophile design), not the "
        "absence of the cultural-footprint Recall pathway."
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.19 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # All 3 charts from build_charts_v19.py at reports/figs/v19/.
        "f1_cp_distribution": (
            "chart_01_cp_distribution.pdf",
            "6_col_v19_cp_distribution",
        ),
        "f2_dissociation_scatter": (
            "chart_02_dissociation_scatter.pdf",
            "6_col_v19_dissociation_scatter",
        ),
        "f3_channel_asymmetry": (
            "chart_03_channel_asymmetry.pdf",
            "6_col_v19_channel_asymmetry",
        ),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    if chart_path is not None and figsize_key is not None:
        ...
        # v0.19 hero slot names — update from v0.18
        is_hero = slot in (
            "f1_cp_distribution",
            "f2_dissociation_scatter",
            "f3_channel_asymmetry",
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.19 protocol version (unchanged from v0.18 — no methodology update in v0.19)
    ...
    citation_text = (
        ...
        "<i>C3 Rescue Attempted: Recognition Ceiling, Third-Substrate "
        "Dissociation, and Channel Asymmetry on an Audiophile Headphones "
        "Substrate: AIAS v0.19</i>. "
        "Third System. thirdsystem.ai/v19-audiophile-headphones (SSRN forthcoming)"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v19] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v19] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V19_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v19"
    # Charts live in reports/figs/v19/ (matches build_charts_v19.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v19")
    output_path = output_path or (V19_DEPOSIT_ROOT / "reports" / "v19_audiophile.pdf")
    base_pdf = output_path.parent / "_v19_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v19] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v19] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="C3 Rescue \u2014 AIAS v0.19 Audiophile Headphones",
    ...
    doc.build(story)
    print(f"[build_report_v19] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v19] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v19] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v19/reports/v19_audiophile.pdf")
    args = ap.parse_args()
    build(
