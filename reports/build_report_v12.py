#!/usr/bin/env python3
"""
v0.12 Three Empirical Regimes (PM Software + Running Shoes + Olive Oil) — typesetting pipeline.

Forked from build_report_v11.py with surgical changes:
  - Imports v12_threeregimes_content instead of v11_pmtrends_content
  - Section label remains "FINDING"
  - HERO_FIGURE_CAPTIONS replaced with v0.12 brand-format figure captions
  - _slot_lookup table updated for the 4 v0.12 brand-format chart filenames
    (2 reused from SSRN paper, 2 new at 6-col width from
    build_charts_v12_threeregimes.py)
  - CHART_FIGSIZE_IN extended with v0.12 figsize keys
  - Header right text, citation, output filename, doc metadata updated
  - REPORT_PROTOCOL_VERSION stays at v1.1
  - build_leaderboards_spread returns [] (v0.12, like v0.10/v0.11, has no
...
Inputs:
  ...
  - osf/v12/figures/chart_v12_*.pdf      (4 brand-format charts; 2 reused
                                          from SSRN paper, 2 produced by
                                          build_charts_v12_threeregimes.py)

Output:
  - osf/v12/reports/v12_threeregimes.pdf

Run for v0.12:
    python3 build_charts_v12_threeregimes.py
    python3 build_report_v12.py

Locked rules followed (same as v0.6\u2013v0.11):
...
# Make `import v12_threeregimes_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v12_threeregimes_content as content  # noqa: E402
...
# Chart figsize spec (inches). v0.12 brand-format report uses 4 charts at 6-col
# width, with varied heights matching the figsize used in
# build_charts_v12_threeregimes.py (and the reused-from-SSRN charts in
# build_charts_v12.py).
CHART_FIGSIZE_IN = {
    ...
    # v0.12 brand-format report keys — 4 charts at 6-col width.
    # Reused from SSRN paper build_charts_v12.py output:
    "6_col_v12_pm_rankshift":     (7.50, 6.50),  # chart_v12_h3_rank_shift_pmsoftware_t1.pdf
    "6_col_v12_running_partial":  (7.50, 5.80),  # chart_v12_h4_partial_residual_running_t1.pdf
    # New, produced by build_charts_v12_threeregimes.py:
    "6_col_v12_scale_mismatch":   (7.50, 6.00),  # chart_v12_threeregimes_scalemismatch_6col.pdf
    "6_col_v12_h6_zones":         (7.50, 4.70),  # chart_v12_threeregimes_h6zones_6col.pdf
}
...
class V12DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Three Empirical Regimes \u00b7 v0.12 \u00b7 11 May 2026")
...
def content_meta_line(doc: V12DocTemplate) -> str:
...
# Brand-format report figure captions for v0.12 charts.
HERO_FIGURE_CAPTIONS = {
    "f1_pm_rank_shift": (
        "Figure 1 \u00b7 PM software rank-shift slope at t<sub size='6'>1</sub>. Each brand's "
        "rank by AI Presence (left) connected to its rank by Trends (right). Steep "
        "slopes indicate construct divergence. Linear and Todoist surface as the v0.11 "
        "diagnostic cases at v0.12 \u2014 the same brands with the same structural positions."
    ),
    "f2_running_partial": (
        "Figure 2 \u00b7 Running shoes partial-correlation residual scatter at t<sub size='6'>1</sub>. "
        "Rank residuals on both axes after OLS on rank-transformed brand age and tier "
        "ordinal. Bivariate <font name='Helvetica'>\u03c1</font> = 0.808 \u2192 partial "
        "<font name='Helvetica'>\u03c1</font> = 0.466 \u2014 brand age and tier mediate approximately "
        "40% of the bivariate correlation."
    ),
    "f3_olive_scale_mismatch": (
        "Figure 3 \u00b7 Premium olive oil matched-subset bar visualisation. Horizontal "
        "bars showing per-brand AI Presence at t<sub size='6'>1</sub>; green circles indicate brands "
        "with measurable Trends signal; copper \u00d7 indicates brands with Trends signal "
        "below the platform's display threshold (EXCLUDED_E1a). The seven scale-mismatch "
        "cases are flagged \u25B6. Scale-mismatch index: 7 / 15 = 46.7% of the matched "
        "subset."
    ),
    "f4_h6_zones_three_cats": (
        "Figure 4 \u00b7 Cross-category H6 diagnostic zones at t<sub size='6'>1</sub>. Linear-style "
        "region (top-left: AI \u2265 50%, Trends \u2264 5) and Todoist-style region "
        "(bottom-right: AI \u2264 5%, Trends \u2265 20) shaded in copper. PM software "
        "(left panel) shows both \u2014 Linear in Linear-style, Todoist in Todoist-style \u2014 "
        "replicating v0.11. Running shoes (right panel) shows Brooks in the Linear-style "
        "zone but no brand in the Todoist-style zone: the asymmetric boundary mismatch."
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.12 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # 2 reused from SSRN paper build_charts_v12.py output:
        "f1_pm_rank_shift":      ("chart_v12_h3_rank_shift_pmsoftware_t1.pdf",         "6_col_v12_pm_rankshift"),
        "f2_running_partial":    ("chart_v12_h4_partial_residual_running_t1.pdf",      "6_col_v12_running_partial"),
        # 2 new from build_charts_v12_threeregimes.py:
        "f3_olive_scale_mismatch": ("chart_v12_threeregimes_scalemismatch_6col.pdf",   "6_col_v12_scale_mismatch"),
        "f4_h6_zones_three_cats":  ("chart_v12_threeregimes_h6zones_6col.pdf",         "6_col_v12_h6_zones"),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    """v0.12 patterns all render on the SPREAD template (full-width frame),
    treated as hero (chart embedded inline in section's text flow)."""
    slot = pattern["chart_slot"]
    is_hero = slot in (
        "f1_pm_rank_shift", "f2_running_partial",
        "f3_olive_scale_mismatch", "f4_h6_zones_three_cats",
    )
...
def build_hypothesis_scoring_story(styles: dict) -> list:
    ...
    # v0.12 has 12 rows (more than v0.11's 6). Slightly tighter cell padding
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    citation_text = (
        ...
        "<i>Three Empirical Regimes: AI Presence \u00d7 Google Trends across PM Software, "
        "Running Shoes, and Olive Oil \u2014 AI Presence Index v0.12 Phase 3 expansion</i>. "
        "Third System. thirdsystem.ai/v12-three-empirical-regimes (SSRN 6748341)"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v12] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v12] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V12_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v12"
    chart_dir = chart_dir or (V12_DEPOSIT_ROOT / "figures")
    output_path = output_path or (V12_DEPOSIT_ROOT / "reports" / "v12_threeregimes.pdf")
    base_pdf = output_path.parent / "_v12_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v12] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "f1_pm_rank_shift", "f2_running_partial",
        "f3_olive_scale_mismatch", "f4_h6_zones_three_cats",
    ]
    ...
    if chart_dir.exists():
        actual_charts = sorted(p.name for p in chart_dir.iterdir()
                                ...
                                and p.name.startswith("chart_v12"))
        unexpected = [n for n in actual_charts if n not in expected_files]
        if unexpected:
            print(f"[build_report_v12] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V12DocTemplate(
        ...
        title="Three Empirical Regimes \u2014 AI Presence Index v0.12",
    ...
    # --- (No leaderboards spread for v0.12, like v0.10/v0.11.) ---
    ...
    doc.build(story)
    print(f"[build_report_v12] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v12] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v12] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v12/reports/v12_threeregimes.pdf")
    args = ap.parse_args()
    build(
