#!/usr/bin/env python3
"""
build_charts_v20.py — Third System brand-format charts for AIAS v0.20 (v2).
...
Produces three PDFs at ~/aias/reports/figs/v20/:
...
Usage:
    python build_charts_v20.py
    python build_charts_v20.py --verdicts <path>
"""
...
DEFAULT_VERDICTS = Path.home() / "aias" / "osf" / "v20" / "v20_verdicts.json"
DEFAULT_OUTPUT_DIR = Path.home() / "aias" / "reports" / "figs" / "v20"
...
def build_chart_01(verdicts: dict, output_path: Path) -> None:
    ...
    apply_editorial_layout(
        ...
        source=("Source: AIAS™ Presence Measurement Protocol v0.20 acquisition (May 2026). "
                "v1.5 methodology per SSRN 6810758. Pre-registration v0.20-prereg-r1."),
    )
...
def build_chart_02(verdicts: dict, output_path: Path) -> None:
    ...
    apply_editorial_layout(
        fig,
        title="Recognition × Recall — Iwachu dissociation on the v0.20 skincare panel",
        subtitle="Iwachu pattern: C_P ≥ 5  ∧  R_cat ≤ 2  (high Recognition, sparse canonical Recall)",
        source=("Source: AIAS™ Presence Measurement Protocol v0.20 (May 2026). "
                "Upstream phases: v0.17 SSRN 6802261, v0.18 SSRN 6806558, v0.19 SSRN 6809182, v1.5 SSRN 6810758."),
    )
    ...
    fig.text(panel_x, panel_y_top, "Iwachu cases (v0.20)",
    ...
    fig.text(0.08, 0.067,
             ...
             f"(Iwachu cases in all 3 cells; cumulative anchor base now spans 4 substrate families).",
...
def build_chart_03(verdicts: dict, output_path: Path) -> None:
    ...
    apply_editorial_layout(
        fig,
        title="Two-channel Recall — R_cat × R_cult on the v0.20 skincare panel",
        subtitle="Three v1.5 dissociation quadrants. Type 2 (cultural-preferred) is the primary v0.20 hypothesis.",
        source=("Source: AIAS™ Presence Measurement Protocol v0.20 (May 2026). "
                "v1.5 two-channel Recall per SSRN 6810758 — first prospective phase."),
    )
    ...
    offsets = [(2.8, 0.0), (3.0, 1.2), (3.0, -1.2)]  # right, right-up, right-down
    ...
    # Bottom headline
    fig.text(0.08, 0.067,
             f"H_Type2_emergence → PARTIAL  "
             f"({sum(1 for c in dissoc['type_2'] if c['cell']=='B')} cases in Cell B). "
             f"Cultural-channel Recall pathway empirically populated for the first time.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build v0.20 charts (v2 — single-chart layouts, side panels)")
    ...
    print("Building v0.20 charts (v2 layout)")
