#!/usr/bin/env python3
"""
v0.19 SSRN paper build pipeline (pandoc + xelatex + Carlito).

Reads:   papers/v0_19/v0_19_ssrn_paper_draft.md
Writes:  papers/v0_19/v0_19_ssrn_paper.pdf

Applies Unicode → LaTeX substitutions for glyphs Carlito lacks. The
substitution table is copy-forwarded verbatim from build_paper_v18.py
per the build_paper_v18 note that the table is exhaustive for AIAS-
paper glyphs and safe to copy-forward to v0.19+.
...
v0.19 substantive notes:
  - The v0.19 draft uses "AND" rather than ∧ in the hypothesis prose
    and chart captions for typographic consistency with the rendered
    brand-format report (chart 2 PDF uses "AND" after the post-build
    Akkurat glyph fix). The substitution table would handle ∧ correctly
    if reintroduced; the choice to use "AND" in v0.19 is editorial.
  - All other Carlito substitutions (≥, ≤, ×, ρ, →) appear in v0.19
    and route through the table identically to v0.18.

Run from project root:
    cd ~/aias
    python3 reports/build_paper_v19.py

Or explicitly:
    python3 reports/build_paper_v19.py \\
        --input papers/v0_19/v0_19_ssrn_paper_draft.md \\
        --output papers/v0_19/v0_19_ssrn_paper.pdf
"""
...
# ---------------------------------------------------------------------------
# Substitution table (verbatim from build_paper_v18.py; ordered — longer/
...
def build_paper(md_path: Path, pdf_path: Path, verbose: bool = True) -> None:
    """Build the v0.19 SSRN paper PDF via pandoc + xelatex."""
    ...
    if verbose:
        print(f"[build_paper_v19] input:  {md_path}")
        print(f"[build_paper_v19] output: {pdf_path}")
        if counts:
            print(f"[build_paper_v19] Unicode → LaTeX substitutions applied:")
            for char, n in sorted(counts.items(), key=lambda kv: -kv[1]):
                latex = dict(UNICODE_TO_LATEX).get(char, '?')
                print(f"  {n:>4}x  {char}  →  {latex}")
        else:
            print(f"[build_paper_v19] No Unicode substitutions needed.")
    ...
    try:
        # --- Pandoc invocation ---
        # The YAML frontmatter in v0_19_ssrn_paper_draft.md carries
        ...
        cmd = [
            ...
            # Resolve relative image paths (e.g. ../../reports/figs/v19/*.pdf)
            ...
            # directory makes `../../reports/figs/v19/chart_NN.pdf` resolve
            # correctly to <project_root>/reports/figs/v19/chart_NN.pdf.
            '--resource-path', str(md_path.parent),
            '--standalone',
            # Leave --toc off: the v0.19 paper is short enough that a TOC
            # adds friction rather than help. Add later if needed.
        ]

        if verbose:
            print(f"[build_paper_v19] pandoc command:")
            print(f"  {' '.join(cmd)}")
        ...
        if verbose and result.stderr.strip():
            ...
            print(f"[build_paper_v19] pandoc warnings:")
            for line in result.stderr.splitlines():
                if line.strip():
                    print(f"  {line}")

        pdf_size_kb = pdf_path.stat().st_size / 1024
        print(f"[build_paper_v19] ✓ PDF written: {pdf_path}  ({pdf_size_kb:.1f} KB)")

        # --- Page count sanity check ---
        try:
            from pypdf import PdfReader
            page_count = len(PdfReader(str(pdf_path)).pages)
            print(f"[build_paper_v19] page count: {page_count}")
            if page_count < 8 or page_count > 30:
                print(f"[build_paper_v19] WARNING: page count outside the 8–30 expected band")
        except ImportError:
            print(f"[build_paper_v19] (pypdf not installed; page count check skipped)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Build v0.19 SSRN paper PDF via pandoc + xelatex + Carlito",
    )
    parser.add_argument(
        '--input', type=Path,
        default=Path('papers/v0_19/v0_19_ssrn_paper_draft.md'),
        help='Path to the markdown source (default: papers/v0_19/v0_19_ssrn_paper_draft.md)',
    )
    parser.add_argument(
        '--output', type=Path,
        default=Path('papers/v0_19/v0_19_ssrn_paper.pdf'),
        help='Path to the output PDF (default: papers/v0_19/v0_19_ssrn_paper.pdf)',
    )
    parser.add_argument(
    ...
    print(f"[build_paper_v19] {check_pandoc_available()}")
