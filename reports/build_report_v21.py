#!/usr/bin/env python3
"""
v0.21 Cosmetics — typesetting pipeline.

Type 2 confirmation phase (n=24, three IL-gradient cells: Prestige / Celebrity DTC
/ Drugstore-mass). Recognition × Recall dissociation construct extends to a fifth
substrate family with Iwachu cases in all three cells (13 total — most in the
program); v1.5's Type 2 quadrant transitions PARTIAL → EMERGED for the first time
with 3 clean Cell B cases (Rare Beauty, Huda Beauty, Kylie Cosmetics) plus a 4th
out-of-cell case (e.l.f. Cosmetics, Cell C). Recognition saturates fully across
all three cells (every brand C_P = 6/6) — v1.5 C2 fails uniformly, routing
Regime 4 to FALSIFIED at the regime-floor. The Phantom Brand Persistence
construct reaches its strongest single demonstration: Glossier surfaces in q6
across all 6/6 panel models, off-panel entirely.
Brand-format report companion to the v0.21 SSRN paper (SSRN TBD).

Forked from build_report_v20.py with surgical changes:
  - Imports v21_cosmetics_content instead of v20_skincare_content
  - Section label remains "FINDING"
  - HERO_FIGURE_CAPTIONS replaced with v0.21 brand-format figure captions (3)
  - _slot_lookup table updated for the 3 v0.21 brand-format chart filenames
    (all produced by build_charts_v21.py at reports/figs/v21/):
  ...
  - CHART_FIGSIZE_IN extended with v0.21 figsize keys (3 entries at 7.5x5.5,
    matching the native figsize used in build_charts_v21.py)
  - Header right text updated: "Type 2 Confirmed · v0.21 · May 2026"
  - Citation updated: v0.21 cosmetics, SSRN TBD
  - Output filename: v21_cosmetics.pdf
  - REPORT_PROTOCOL_VERSION remains v1.5 (replicates v1.5; not a new methodology phase)
  - Chart directory: reports/figs/v21/
  - Page count target band: 8-14 pages (v0.21 has 3 findings, same as v0.19/v0.20)

Inputs:
  - brand/third_system_brand.json        (v1.5)
  - brand/design_tokens_template.json    (IDML extract)
  - reports/figs/v21/chart_01_*.pdf      (3 brand-format charts; all from
                                          build_charts_v21.py)

Output:
  - osf/v21/reports/v21_cosmetics.pdf

Run for v0.21:
    python3 ~/aias/scripts/build_charts_v21.py
    python3 ~/aias/reports/build_report_v21.py

Locked rules followed (same as v0.6 - v0.19):
...
# Make `import v21_cosmetics_content` work regardless of working directory.
sys.path.insert(0, str(SCRIPT_DIR))
import v21_cosmetics_content as content  # noqa: E402
...
class V15DocTemplate(BaseDocTemplate):
    ...
    def _body_chrome(self, canvas: Canvas, doc):
        ...
        c.setFillColor(HexColor(self.palette.soft_black))
        # v0.21 header right text — updated from v0.20
        c.drawRightString(PAGE_W - MARGIN, header_y - 8,
                           "Type 2 Confirmed \u00b7 v0.21 \u00b7 May 2026")
...
def build_lead_story(styles: dict[str, ParagraphStyle]) -> list:
    ...
    s.append(Spacer(1, 4))
    # Hint next template BEFORE EXEC_SUMMARY can auto-paginate, so any
    # overflow goes to body (2-col) rather than another lead-template page
    # (whose lead_top frame is a full-width 396pt single-column block).
...
# v0.21 brand-format report figure captions for the three v0.21 charts.
HERO_FIGURE_CAPTIONS = {
    "f1_cp_distribution": (
        "Figure 1 \u00b7 Phase A Recognition (C_P) score per brand, per cell. "
        "Each point is one brand; the cosmetics substrate produces uniform "
        "saturation across all three cells. Cell A (Prestige): all eight "
        "brands at C_P = 6/6 (MAC Cosmetics, NARS, Bobbi Brown, Tom Ford "
        "Beauty, Giorgio Armani Beauty, Hourglass, Chantecaille, Laura "
        "Mercier). Cell B (Celebrity DTC / cult): all eight at 6/6 (Rare "
        "Beauty, Fenty Beauty, Haus Labs, Pat McGrath Labs, Charlotte "
        "Tilbury, Huda Beauty, Kylie Cosmetics, Anastasia Beverly Hills). "
        "Cell C (Drugstore / mass): all eight at 6/6 (Maybelline, "
        "L\u2019Or\u00e9al Paris, CoverGirl, Revlon, NYX Professional "
        "Makeup, e.l.f. Cosmetics, Wet n Wild, Milani Cosmetics). Distinct "
        "count = 1 and modal share = 1.000 in every cell; v1.5 C2 fails "
        "uniformly. Cosmetics is a maximally-coded category at the "
        "Recognition channel \u2014 the substrate where discrimination "
        "must come entirely from Recall."
    ),
    "f2_dissociation_scatter": (
        "Figure 2 \u00b7 Recognition \u00d7 Recall dissociation scatter \u2014 "
        "cumulative anchor base across five substrate families. Each colored "
        "point is one v0.21 cosmetics brand: Phase A C_P (x-axis, 0\u20136) "
        "versus Phase B category-anchored mention count (y-axis, 0\u201318). "
        "Gray ghost markers show v0.17 Japanese-cell, v0.18 indie-fragrance, "
        "v0.19 audiophile, and v0.20 skincare Iwachu cases. Shaded quadrant: "
        "Iwachu-pattern (C_P \u2265 5 \u2227 R_cat \u2264 2). v0.21 "
        "contributes 13 cases distributed across all three cells \u2014 the "
        "largest single-phase Iwachu count in the program. The AIAS\u2122 "
        "v1.4/v1.5 multi-component construct now spans five substrate "
        "families with multi-cell distribution; the cross-substrate "
        "generalization claim for AIAS\u2122 1.0 reaches its strongest "
        "evidentiary state."
    ),
    "f3_channel_asymmetry": (
        ...
        "\u00d7 6 models = max 18). Lower-right shaded quadrant: Type 1 "
        "(canonical-preferred; R_cat \u2265 5 \u2227 R_cult \u2264 2). "
        "Upper-left quadrant: Type 2 (cultural-preferred; R_cat \u2264 2 "
        "\u2227 R_cult \u2265 5). Type 1 cases in v0.21: Bobbi Brown "
        "(R_cat = 8, R_cult = 0) and Laura Mercier (R_cat = 5, R_cult = 0), "
        "both Cell A. Four Type 2 cases: Rare Beauty (R_cat = 1, R_cult = "
        "17 \u2014 the program\u2019s textbook dissociation), Huda Beauty "
        "(R_cat = 0, R_cult = 11), and Kylie Cosmetics (R_cat = 0, R_cult "
        "= 5) in Cell B; e.l.f. Cosmetics (R_cat = 1, R_cult = 9) in Cell C "
        "\u2014 the first out-of-cell Type 2 surface in the program. Cell B "
        "Type 2 count = 3 clears the EMERGED threshold; the Type 2 quadrant "
        "transitions PARTIAL (v0.20) \u2192 EMERGED (v0.21)."
    ),
}


def _slot_lookup(slot_key: str) -> tuple[str | None, str | None]:
    """Map a v0.21 brand-format slot_key to (filename, figsize_key)."""
    table = {
        # All 3 charts from build_charts_v21.py at reports/figs/v21/.
        "f1_cp_distribution": (
            "chart_01_cp_distribution.pdf",
            "6_col_v21_cp_distribution",
        ),
        "f2_dissociation_scatter": (
            "chart_02_dissociation_scatter.pdf",
            "6_col_v21_dissociation_scatter",
        ),
        "f3_channel_asymmetry": (
            "chart_03_channel_asymmetry.pdf",
            "6_col_v21_channel_asymmetry",
        ),
    }
...
def build_pattern_unified(pattern: dict, styles: dict,
    ...
    if chart_path is not None and figsize_key is not None:
        ...
        # v0.21 hero slot names — same as v0.18/v0.19/v0.20 (3 findings pattern)
        is_hero = slot in (
            "f1_cp_distribution",
            "f2_dissociation_scatter",
            "f3_channel_asymmetry",
...
def build_closing_story(styles: dict, brand: dict) -> list:
    ...
    # v0.21 protocol version — replicates v1.5 (no methodology update in v0.21)
    REPORT_PROTOCOL_VERSION = "v1.5"
    ...
    citation_text = (
        ...
        "<i>Type 2 Confirmation, Recognition Ceiling, and Phantom Brand "
        "Persistence on a Cosmetics IL-Gradient Substrate: AIAS v0.21</i>. "
        "Third System. thirdsystem.ai/v21-cosmetics (SSRN TBD)"
    )
...
def build(*, debug_layout: bool = False,
    ...
    print(f"[build_report_v21] palette: indigo={palette.indigo}, "
          f"soft_black={palette.soft_black}, paper={palette.paper}")
    print(f"[build_report_v21] typography: {font.name} "
          f"(brand_primary={font.is_brand_primary})")

    V21_DEPOSIT_ROOT = AIAS_ROOT / "osf" / "v21"
    # Charts live in reports/figs/v21/ (matches build_charts_v21.py default).
    chart_dir = chart_dir or (AIAS_ROOT / "reports" / "figs" / "v21")
    output_path = output_path or (V21_DEPOSIT_ROOT / "reports" / "v21_cosmetics.pdf")
    base_pdf = output_path.parent / "_v21_base.pdf"
    output_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"[build_report_v21] chart pre-flight (looking in {chart_dir})")
    expected_slots = [
        "f1_cp_distribution", "f2_dissociation_scatter", "f3_channel_asymmetry",
    ]
    expected_files = set()
    ...
    if chart_dir.exists():
        ...
        if unexpected:
            print(f"[build_report_v21] chart files in dir not used by brand-format report:")
            for n in unexpected:
                print(f"  [UNUSED ] {n}")
    ...
    doc = V15DocTemplate(
        ...
        title="Type 2 Confirmed \u2014 AIAS v0.21 Cosmetics",
    ...
    doc.build(story)
    print(f"[build_report_v21] base PDF written: {base_pdf} "
    ...
    print(f"[build_report_v21] FINAL PDF written: {output_path}")

    final_pages = len(PdfReader(str(output_path)).pages)
    print(f"[build_report_v21] page count: {final_pages}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=None,
                     help="Output PDF path. Defaults to ~/aias/osf/v21/reports/v21_cosmetics.pdf")
    args = ap.parse_args()
    build(
