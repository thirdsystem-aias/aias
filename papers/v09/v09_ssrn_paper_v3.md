...
![Per-brand drift across all five categories on the matched subset, t₁→t₂. Each point is one registered brand in one category; horizontal axis is t₁ Presence and vertical axis is the t₁→t₂ delta in percentage points. The shaded band marks H1 confirmation thresholds at ±5pp and ±10pp. 88.3 percent of the 103 brand-level deltas land within ±5pp; 99.0 percent within ±10pp; no brand exceeds ±20pp. The empirical noise floor of one-week AI Presence measurement on the matched subset is approximately ±5 percentage points for the modal brand.](figures/chart_v09_h1_drift_scatter_6col.pdf){#fig:h1-drift width=100%}
...
![Top-3 brands at t₁ remain in top-5 at t₂ across all five categories. Each panel shows one category; horizontal bars are sorted by t₂ Presence; the asterisk and copper coloring mark brands that ranked in the t₁ top-three. Leaderboard composition is preserved with no exceptions across the seven-day interval. Top-of-leaderboard stability is the most directly category-management-relevant of the v0.9 findings.](figures/chart_v09_h2_leaderboard_6col.pdf){#fig:h2-leaderboard width=85%}
...
![Within-category brand-presence variance ordering preserves between t₁ and t₂. Bars show within-category standard deviation of per-brand Presence at each wave; categories sorted by t₂ variance. Personal finance is the highest-dispersion category and running shoes the lowest. The Spearman rank correlation across the five categories between t₁ and t₂ is ρ = 0.8, above the H3 confirmation threshold of 0.7.](figures/chart_v09_h3_within_cat_variance_6col.pdf){#fig:h3-variance width=100%}
...
![Mint phantom-brand persistence at gross Presence in personal finance. The shaded band marks the H4 ±5pp stability threshold around t₁'s 44.8 percent. Mint's gross Presence at t₂ is 41.7 percent — a Δ of −3.1pp, well within the band. The finding confirms the predicted-stability outcome: a defunct brand's recommendation-slot persistence does not decay measurably across a seven-day interval. (For the v0.7 reframe distinguishing gross Presence from naive-phantom rate, see §3.4 narrative.)](figures/chart_v09_h4_mint_persistence_4col.pdf){#fig:h4-mint width=60%}
...
![H5 Pattern 4 sensitivity. Spanish olive oil and K-beauty skincare aggregate Presence against the v0.9 thresholds (12.5pp and 5pp respectively, dashed). The strict Spanish cohort (Castillo de Canena, Núñez de Prado) measures 11.5pp and passes; the inclusive cohort (adding Graza) measures 14.2pp and breaches. K-beauty (Beauty of Joseon alone) measures 1.0pp, well below the 5pp threshold. The 2.7-point swing on the Spanish bar between strict and inclusive readings is entirely Graza, the diagnostic case that isolates brand-marketing-language from country-of-origin.](figures/chart_v09_h5_pattern4_sensitivity_6col.pdf){#fig:h5-pattern4 width=100%}
...
![Cross-model spread stability across the seven-day interval. Each bar is one category's Pearson correlation between t₁ and t₂ on per-brand cross-model spread (Sonnet minus gpt-5.4-mini); the dashed line marks the 0.7 H6 confirmation threshold. All five categories clear the threshold. Cross-model relative behavior — what each model emphasizes versus its companion within a category — is preserved across the wave interval.](figures/chart_v09_h6_cross_model_spread_6col.pdf){#fig:h6-spread width=100%}
...
![Post-hoc Pattern 1 cross-model spread by category replicates v0.6's qualitative ordering. Bars show per-category mean |Sonnet − gpt-5.4-mini| in percentage points at t₁ and t₂. The Spearman rank correlation between t₁ and t₂ category orderings is ρ = 0.800; between v0.9's t₂ ranking and v0.6's narrative ordering (personal finance > olive oil > project management > skincare > running), ρ = 0.900. Personal finance widens to the most-fragmented-discourse category at t₂; project management narrows. Both shifts are within H1's empirical noise floor but in directions that strengthen v0.6's stated ordering.](figures/chart_v09_pattern1_spread_6col.pdf){#fig:pattern1-spread width=100%}
...
![Exploratory mode-distribution shifts on the matched subset. Stacked bars show the share of model responses classified as brand-mode (specific named brand recommendation), mixed, component (non-brand attribute), authority (third-party reference), or refusal — t₁ left, t₂ right per category. Project management gains brand-mode share (+7.3pp) and loses authority-mode share (−4.1pp); olive oil loses brand-mode share (−8.3pp) with gains in mixed and component modes; skincare loses component-mode share (−6.2pp). The shifts are reported as a calibration baseline for a future v0.10 pre-registered mode-stability hypothesis.](figures/chart_v09_mode_distribution_6col.pdf){#fig:mode-dist width=100%}
...
![v0.6 framework pattern replication at v0.9, summary matrix. Pattern 1 (cross-model spread) replicates across all five categories; Pattern 4 (discourse-language bias) replicates strict in the two cross-lingual categories where it was tested; Pattern 6 (phantom-brand) replicates in personal finance where it was tested (Mint). The bottom row shows brand-mode share at t₂ as a category-level summary signal. Patterns 2, 3, and 5 are not directly measured at v0.9 (see §7 for v0.10+ pre-registration candidates).](figures/chart_v09_pattern_matrix_6col.pdf){#fig:pattern-matrix width=100%}
