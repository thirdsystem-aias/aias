...
Methodology locked at \texttt{v1.8-prereg-r2}\\
Falsification and redefinition escalation recorded at \texttt{v1.8-prereg-r3}
...
**Paper status:** Working paper in the AIAS\texttrademark{} Measurement Program. Pre-registered; methodology locked at git tag `v1.8-prereg-r2`, falsification and redefinition escalation recorded at `v1.8-prereg-r3`. Data and code deposited at OSF (`osf.io/ec6wh`).
...
![Defunct brands return undefined consistency. Cell-D automotive brands record zero recall across the panel and fall below the floor, shown against the in-market recall spread for reference.](../../reports/figs/v1_8/fig_03_phantom_null.pdf){#fig:phantom width=100%}
...
![Many recognized brands have no defined consistency. Defined against undefined under the locked floor, by substrate; the undefined brands are recognized but rarely recalled.](../../reports/figs/v1_8/fig_01_cpc_defined.pdf){#fig:defined width=100%}
...
![The coupling is arithmetic. Each brand's coefficient of variation against its mean recall, with the Poisson prediction in which the coefficient of variation falls as the reciprocal of the square root of the mean; the points track the curve, and the rank correlation between the two is $-0.77$.](../../reports/figs/v1_8/fig_04_cv_mean_coupling.pdf){#fig:coupling width=100%}

![Consistency tracks Presence rather than dissociating from it. Each brand's consistency score against its composite presence, by substrate, with the pooled rank correlation annotated; at recall counts between zero and six the coefficient of variation approximates the reciprocal of the square root of the mean.](../../reports/figs/v1_8/fig_02_cpc_dissociation.pdf){#fig:dissociation width=100%}
...
**Data and code availability.** Pre-registration artifacts, scoring code, the scored dataset, and figures are deposited at OSF (`osf.io/ec6wh/methodology/v1_8`). Consistency values reconcile with the pilot's recorded output to a maximum absolute difference of $2\times10^{-16}$.
